...
| `.tbw-visibility-move-menu` | Click-only move menu (SC 2.5.7) |
| `.tbw-visibility-move-menu-item` | An entry in that menu |
...
movement never starts a drag, so the click and the drag never conflict. See the
[accessibility guide](/grid/guides/accessibility/) for every drag surface and its alternative.
