...
- DECIDED (#326, May 2026): browsers cap a single element's rendered height at ~33.5M px (Chromium `2^25`); above the cap the faux-vscroll spacer would silently truncate (with `rowHeight: 34`, hit at ~986,895 rows), making the dataset's tail unreachable via native scrollbar / `Ctrl+End` / `scrollToRow(N)`. Fix: hybrid pixel-vs-fraction mapping. `calculateTotalSpacerHeight` clamps the row-content portion to `MAX_ELEMENT_HEIGHT_PX` (33_554_400) and stores a `ScrollMapping` on `VirtualState` (`rawContentHeight`, `spacerHeight`, `viewportHeight`, `capped`). Three call sites translate between coordinate spaces: `refreshVirtualWindow` and the `grid.ts` scroll listener call `toVirtualScrollTop(scrollTop, mapping)` before binary-searching the position cache or dividing by `rowHeight`; `focus-manager.scrollToRow` calls `fromVirtualScrollTop(target, mapping)` to translate the row offset back into spacer-space `scrollTop`. Identity (no transform) when `rawContentHeight ≤ MAX_ELEMENT_HEIGHT_PX` — sub-cap behavior unchanged. Public surface: `MAX_ELEMENT_HEIGHT_PX`, `ScrollMapping`, `computeScrollMapping`, `toVirtualScrollTop`, `fromVirtualScrollTop` re-exported from `public.ts`. `computeVirtualWindow` accepts an optional `maxSpacerHeight` (default `MAX_ELEMENT_HEIGHT_PX`, `Infinity` opts out). The `tbw-scroll` event detail and plugin `onScroll` event continue to report **native** spacer-space `scrollTop` / `scrollHeight` / `clientHeight` — these stay self-consistent and match `fauxScrollbar.scrollTop`. Files: [virtualization.ts](libs/grid/src/lib/core/internal/virtualization.ts), [virtualization-manager.ts](libs/grid/src/lib/core/internal/virtualization-manager.ts), [focus-manager.ts](libs/grid/src/lib/core/internal/focus-manager.ts), [grid.ts](libs/grid/src/lib/core/grid.ts) scroll listener. Tests: `virtualization.spec.ts > Scroll Mapping Tests` + `__tests__/integration/scroll-mapping-cap.spec.ts`. RULED OUT: dropping the spacer entirely in favor of pure fractional mapping (loses pixel-accurate behavior for the 99% case and breaks wheel/keyboard step granularity below the cap).
