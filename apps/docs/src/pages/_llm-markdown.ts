...
export interface AgentMarkdownOptions {
  ...
  /**
   * Absolute site origin (e.g. `https://toolboxjs.com`) prepended to every
   * internal doc link rewritten by `hasDoc`. Agents are explicitly told to
   * DOWNLOAD these corpora and grep them locally, at which point a root-relative
   * `/grid/core.md` is unresolvable — there is no origin to resolve it against.
   * Always pass this from an endpoint. When omitted, links stay root-relative.
   */
  origin?: string;
  /**
 ...
 * anchors, and unknown paths are returned unchanged. `origin` makes the result
 * absolute so the link still resolves in a downloaded copy of the corpus.
 */
function rewriteDocLink(url: string, hasDoc?: (slug: string) => boolean, origin = ''): string {
  ...
  return origin.replace(/\/+$/, '') + '/' + slug + '.md' + suffix;
}
...
export function mdxToAgentMarkdown(raw: string, opts: AgentMarkdownOptions): string {
  ...
  const transformed = segments.map((seg) => {
    ...
    out = out.replace(/\]\((\/[^)\s]+)\)/g, (full, url: string) => {
      const rewritten = rewriteDocLink(url, opts.hasDoc, opts.origin);
