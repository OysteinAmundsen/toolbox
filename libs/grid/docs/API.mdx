...
3. `rowId` property on the row object
4. Falls back to row index (not recommended for mutable data)
...
// Track the update source for debugging/logging
grid.updateRow('EMP-123', { status: 'inactive' }, 'api-sync');
grid.updateRows(updates, 'websocket');
```
