...
- TENSION: a fast scroll-jump past the render window before `onScrollRender` fires leaves the clone unprimed until the user scrolls back. In `'stack'` mode this is user-visible: `refreshDisplay` appends only indices that HAVE a clone, so a fast scroll silently yields a stack of 1 even though `computeDisplay` qualified `maxStacked` entries. Reproduced on CI at ~75 px per wheel tick (promo scene `Sticky section rows` — it now scrolls at ~25 px/tick for that reason). A fix would require synthesising from data (re-couples to `core/internal/rows.ts`).
