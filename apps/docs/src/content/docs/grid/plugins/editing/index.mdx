...
You can instead pass `columns={[ … ]}` and `editing="dblclick"` as individual props on `<DataGrid>`. That form is idiomatic React, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
  <TabItem label="Vue">
```html
...
<template>
  <TbwGrid :rows="data" :grid-config="gridConfig" @cell-commit="onCellCommit" style="height: 400px" />
</template>
```

:::note[Alternative: prop + light-dom columns]
You can instead bind `:editing="'dblclick'"` and declare columns with `<TbwGridColumn>` children. That form is idiomatic Vue, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
...
You can instead import `GridEditingDirective` and bind `[editing]` directly on `<tbw-grid>`. That form gives a compile-time check that the feature is imported, but `gridConfig.features` above is the recommended, framework-consistent approach.
:::
  </TabItem>
</Tabs>
...
See [Editor Parameters](#editor-parameters) for the full `editorParams` shape per type. The select editor's choices come from the column-level `options` property (not `editorParams`); `editorParams.includeEmpty`/`emptyLabel` only control the leading blank entry.

For anything beyond these — overlay pickers, multi-field composites, or framework components — provide a custom `editor` on the column (see [Custom Editors](#custom-editors) below) or, in a framework adapter, the adapter's editor base class.
...
To dynamically add or remove rows from the grid, assign a new array to the `rows` property. The grid reactively updates to display the new data. Use a custom `renderer` in an "Actions" column to provide a delete button for each row.
...
// Listen for per-cell commits (inline edit + updateRow/updateRows)
grid.on('cell-commit', (e) => {
  console.log('Committed from:', e.detail.source); // 'user' | 'api' | 'paste' | 'history'
});

// Listen for value-change notifications
grid.on('cell-change', (e) => {
  console.log('Cell changed:', e.detail.field, e.detail.value);
});
```

:::note
Programmatic updates via `updateRow()`/`updateRows()` flow through the same commit pipeline as inline edits (validation, dirty tracking, undo/redo integration), then emit `cell-change` as the value-change notification.
:::
...
function MyGrid({ data }) {
  return <DataGrid rows={data} gridConfig={gridConfig} />;
}
```
  </TabItem>
  <TabItem label="Vue">
...
@Component({
  imports: [Grid],
  template: `<tbw-grid [rows]="data" [gridConfig]="gridConfig" />`
})
export class MyGridComponent {
  data = [...];
  gridConfig: GridConfig = {
...
:::note
`editor`, `editorParams`, `editable`, `multi` and `nullable` are contributed to `TypeDefault` by
this plugin. They appear on `gridConfig.typeDefaults` entries only when the editing plugin's types
are in scope — importing `@toolbox-web/grid/features/editing` (or the matching adapter feature entry)
is what makes them type-check.
:::
...
:::caution[Editor markup is a code-trust boundary]
Editor markup is **not** passed through the grid's HTML sanitizer. It cannot be — the sanitizer
strips `input`, `select`, `textarea` and `button`, which is everything an editor is made of.

That is safe as long as editor definitions come from your **source code**: an `editor` function or
a light-DOM `<template editor>` is code, the same as any other component you ship. Interpolated
row values are still HTML-escaped by the compiled template, so untrusted *data* cannot break out.

It stops being safe if editor definitions ever become **data**. Never build an `editor` function,
an editor `template`, or the HTML inside one from a serialized config, a database column, a URL
parameter, or any other remote source — that turns editors into an XSS sink.
:::
...
The `ctx` object passed to custom editors carries the cell's `value`, `row`, `column`, `field`
and `rowId`, plus the callbacks that drive the edit session: `commit(value)` to accept,
`cancel()` to discard, `updateRow(changes)` to write other fields on the same row through the
commit pipeline, and `onValueChange(cb)` to stay in sync when the value changes underneath you.

See [`ColumnEditorContext`](/grid/api/core/interfaces/columneditorcontext/) for the full typed
...
:::tip[Masked input and schema validation]
Built-in editors are deliberately plain `<input>` / `<select>` elements. For phone/IBAN masks or Zod-style schemas, see [Recipes: Input masks & validation](/grid/recipes/input-masks-validation/).
:::
...
- **[Recipes: Editable grid with undo](/grid/recipes/editable-grid-with-undo/)** — Editing, undo/redo, and dirty tracking wired together
