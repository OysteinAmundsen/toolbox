...
description: Practical recipes for combining plugins and grid features. Covers data browsing, editable grids, master-detail, grouping, export, and more.
---

import { LinkCard, CardGrid, Tabs, TabItem } from '@astrojs/starlight/components';

Real-world grids rarely use a single plugin in isolation. This guide shows **tested combinations** that solve everyday requirements.
...
grid.gridConfig = {
  ...
  plugins: [
    new SelectionPlugin({ mode: 'row', multiSelect: true }),
...
grid.gridConfig = {
  ...
  plugins: [
    ...
    new UndoRedoPlugin(),                    // Must come after EditingPlugin
...
:::danger
`EditingPlugin` must be loaded **before** `UndoRedoPlugin`. Wrong order throws a runtime error.
:::
...
grid.gridConfig = {
  columns,
  plugins: [
    new GroupingRowsPlugin({ groupOn: (row) => [row.department], defaultExpanded: true }),
