...
| —               | Root devDependencies removed: `typedoc-plugin-markdown` (no `plugin` key in `typedoc.json`), `vite-tsconfig-paths` (no vite config references it). **`typescript-eslint` was also removed and had to be restored** — `@nx/eslint-plugin`'s `flat/typescript` + `flat/javascript` configs `require("typescript-eslint")` without declaring it, so removing it broke the Nx project graph in CI (see `build-and-deploy.md`). |
...
**Parity gaps found and closed while writing the spec:** the allowlist started with `rowDragDrop`
