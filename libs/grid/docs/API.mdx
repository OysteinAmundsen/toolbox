...
> **Note:** `cell-commit`, `row-commit`, and `changed-rows-reset` events require the [EditingPlugin](?path=/docs/grid-plugins-editing--docs). The `cell-change` event is a core event that fires for any programmatic row updates via the Row Update API.
