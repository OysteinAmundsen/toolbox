...
> **Note:** `cell-commit`, `row-commit`, `changed-rows-reset`, `edit-open`, `before-edit-close`, and `edit-close` events require the [EditingPlugin](?path=/docs/grid-plugins-editing--docs). The `cell-change` event is a core event that fires for any programmatic row updates via the Row Update API.
