...
When one `GRID_STATS` field stands in for **several** packages (e.g. one range printed for both `react` and `react-dom`, or the `@toolbox-web/grid` peer shown on every adapter row), route it through `sharedRange()` so the docs build **throws** if those ranges ever diverge. Reading one package and rendering it as if it covered the others is exactly the drift the module exists to prevent.
...
## Prefer absolute internal links — relative links resolve one level too deep

Every docs page is served with a trailing slash, so a relative link from a non-`index` page resolves _inside_ that page's own URL: `./multi-version` written in `guides/theming.mdx` becomes `/grid/guides/theming/multi-version/`, not `/grid/guides/multi-version/`. Relative links only work from `index.mdx` pages (where `./interfaces/foo/` and `../server-side/` are correct, as in every `plugins/*/index.mdx`). **From any non-index page, always write the absolute path** (`/grid/guides/multi-version/`). Detect with `grep -rn '](\./\|](\.\./' apps/docs/src/content/docs` and check whether the containing file is an `index.mdx`.
