...
> **Column shorthand.** The `field:type` shorthand keeps prototyping markup terse — `field="salary:number"` is equivalent to `field="salary" type="number"` with an auto-generated `header`. The recognized suffixes are `string`, `number`, `boolean`, `date`, `datetime`, and `currency`. Of these, only `string`/`number`/`boolean`/`date` are `PrimitiveColumnType` built-ins; `datetime` and `currency` are simply expanded to those type strings (accepted because custom type strings are allowed, but they only alter rendering when a matching `typeDefaults` entry or plugin is registered). Any other colon-containing value is treated as a literal field name. The same shorthand works in the JSON `columns` attribute: `<tbw-grid columns='["id:number", "name", "email"]'>`. A non-empty explicit `type`/`header` attribute wins over the shorthand-derived value.
