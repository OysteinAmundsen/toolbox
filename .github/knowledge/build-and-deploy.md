...
- STORAGE: `actions/cache` keyed `bench-baseline-${{ runner.os }}-${{ github.base_ref || github.ref_name }}-<sha>` with restore-key prefix `bench-baseline-${{ runner.os }}-${{ github.base_ref || github.ref_name }}-`. **No commits, no artifacts-as-baseline, no Nx Cloud dependency.** Path saved/restored is `tmp/bench-baseline.json`; on main/next push the job copies `tmp/bench-current.json` → baseline path before saving so the next restore lands at the path the comparison expects. INVARIANT: `main` and `next` MUST live in separate cache namespaces — `github.base_ref` is set on PRs to the target branch, falling back to `github.ref_name` on push events. A hard-coded `-main-` namespace mixes the two perf profiles and PRs against `next` get wrong-baseline noise.
