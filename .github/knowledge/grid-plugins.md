...
- DECIDED (Aug 2026, PR #464 review follow-up): the ~13 `this.grid as unknown as GridHost` casts in plugins **stay**, each centralized in one `#internalGrid` accessor per plugin. WHY: `GridElement extends GridElementRef` (`core/plugin/types.ts`) and `GridHost = InternalGrid & HTMLElement` (`core/types.ts`) are parallel, unrelated descriptions of the same runtime object, so a single `as` fails comparability. RULED OUT: (a) casting from `gridElement` — breaks 73 tests, adds a runtime `_hostElement` coupling for zero functional gain; (b) `interface GridElement extends GridElementRef, InternalGrid` — same-named members conflict (`_visibleColumns: ColumnConfig[]` vs `ColumnInternal<T>[]`, `rows: unknown[]` vs `T[]`) and TS requires identical types; (c) hoisting one `protected get internalGrid()` onto `BaseGridPlugin` — the base-class member name is unmanglable at every call site, whereas per-plugin `#internalGrid` mangles to one char (editing has only ~500 B of budget headroom). Removing them properly requires reconciling `GridElementRef` vs `InternalGrid` — a type-architecture change, not a cleanup.
