...
- DECIDED (Apr 2026): grid renders into **light DOM**, not shadow DOM. `#renderRoot` returns `this`. Plugins and internal code MUST query the host directly (`gridEl.querySelector('.rows-body')`) — `gridEl.shadowRoot` is always null. The legacy term "shadow DOM tree" in this file refers to the _render tree structure_, not an actual ShadowRoot. Rationale: simpler styling story (user CSS + theme variables work without `::part()` everywhere), easier integration with frameworks that don't play well with shadow boundaries, no slotting complexity. Trade-off: no encapsulation — plugin styles use `tbw-grid` selector prefix to scope.
- DECIDED (Apr 2026): `aria-multiselectable` belongs on the element carrying `role="grid"` (`.rows-body`), NOT on the host. SelectionPlugin sets it in `#applySelectionClasses` based on `multiSelect` (default `true`); detach removes it. Setting it on the host is invalid per WAI-ARIA — assistive tech only checks the role-bearing element.
- DECIDED (Apr 2026): `dataLoaded` a11y announcement only fires when `sourceRowCount` changes between `_emitDataChange()` calls. Internal sort/filter/edit emits leave sourceRowCount unchanged, so they don't spam the live region or conflict with their own dedicated announcements (`sortApplied`, `filterApplied`, `editingCommitted`). Initial `0 → 0` mount transition is suppressed; later increases from 0 announce normally. State tracked via `AriaState.lastAnnouncedSourceCount`.
- DECIDED (Apr 2026): `aria-busy` toggles on the host alongside the `loading` HTML attribute in the `loading` setter. Plugins doing their own async work (e.g., `FilteringPlugin` server-side filter) also set `aria-busy` directly on the host while the request is in flight; clear it on completion.
