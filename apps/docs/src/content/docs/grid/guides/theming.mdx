...
Override CSS custom properties on the grid element. Prefer the
`[data-tbw-grid]` attribute selector over the bare `tbw-grid` tag — it also
matches version-suffixed tags when [multiple grid versions](./multi-version)
share the page:
