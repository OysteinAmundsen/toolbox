...
```typescript
import { ColumnVirtualizationPlugin } from '@toolbox-web/grid/all';
...
### ❌ Bad: Complex Operations in Renderer
...
```typescript
// Log render timing
grid.addEventListener('render-complete', (e) => {
  console.log('Render time:', e.detail.duration);
});

// Monitor scroll performance
let lastScroll = performance.now();
grid.shadowRoot?.querySelector('.scroll-container')?.addEventListener('scroll', () => {
