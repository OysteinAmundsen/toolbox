 ...
 *
 * Called by the Angular `Grid` directive's rows effect to decide whether to
 * call `grid.updateRows()` (value-only, fast path) or `grid.rows =` (full
 * replace). Kept as a pure helper so it can be unit-tested without Angular.
 */
