...
**Problem**: You imported `GridElement` but public API methods are missing.

**Solution**: Use `DataGridElement` instead. There are two similarly-named types:

- **`DataGridElement`** - The public consumer type with all API methods (`ready()`, `columnState`, `toggleToolPanel()`, etc.)
- **`GridElement`** - An internal type for plugin authors (minimal interface)

```typescript
// ❌ Wrong - GridElement is for plugin authors
import type { GridElement } from '@toolbox-web/grid';
const grid = document.querySelector('tbw-grid') as GridElement;
grid.columnState; // Error!

// ✅ Correct - DataGridElement is the public type
import type { DataGridElement } from '@toolbox-web/grid';
...
**Solution**: Add `CUSTOM_ELEMENTS_SCHEMA` to your component:
...
grid.rows = data;
grid.addEventListener('render-complete', handler);
...
**Cause**: The grid uses Shadow DOM, which encapsulates styles.
...
```css
tbw-grid {
  --tbw-row-hover-bg: #e0f7fa;
  --tbw-cell-padding: 12px 16px;
}
```

2. **Use `::part()` selectors** (limited):
...
2. **Inspect the DOM**: Use browser DevTools to examine the shadow DOM
...
```typescript
// Grid version
import { version } from '@toolbox-web/grid';
console.log('Grid version:', version);
