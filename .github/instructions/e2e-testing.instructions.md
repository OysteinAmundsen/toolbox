...
| Sticky rows           | Clones satisfy `dataRows()`/`rowCount()` — filter with `:not(.tbw-sticky-row)`. Switching `mode` rebuilds the dataset and resets scroll: wait for the clone count to drop to `0` **before** scrolling again. In `stack` mode scroll GENTLY (~25 px per wheel tick): clones are captured from the live DOM in `afterRender`, so a section that scrolls past between two render frames is never cloned and is silently dropped from the stack — a fast wheel yields a permanent stack of 1 on a loaded runner |
