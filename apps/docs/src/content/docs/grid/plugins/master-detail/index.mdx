...
When `ServerSidePlugin` is loaded, MasterDetailPlugin can fetch detail data from the server on demand. On detail expand, it queries `datasource:fetch-children` and renders the result using `detailRenderer`. When ServerSide is not present, `detailRenderer` works synchronously as today.
...
**Data flow:**
1. ServerSide fetches rows → renders normally (MasterDetail does not claim root data)
2. On detail expand → MasterDetailPlugin queries `datasource:fetch-children` with `{ source: 'master-detail', row, rowIndex }`
3. ServerSide calls `getChildRows()` → broadcasts `datasource:children`
4. MasterDetailPlugin stores the data and re-renders the detail panel
5. `detailRenderer` uses `getDetailData(rowIndex)` to access the async data
