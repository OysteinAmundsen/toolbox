...
- **[Server-Side](/grid/plugins/server-side/)** — Row grouping requires the full dataset to compute group boundaries. Server-side lazy loading provides rows in blocks, so groups cannot be built client-side.
:::

A development-mode warning is shown if incompatible plugins are loaded together.

:::tip[Works with Master-Detail]
Row Grouping works seamlessly with the [Master-Detail](../master-detail/) plugin. When both are enabled, data rows within expanded groups show the master-detail toggle, while group header rows (which are synthetic) do not. Use `expandOnRowClick: true` on master-detail safely — group header clicks are ignored.
:::
