...
SC 2.5.8 is not conditional on the pointer type, so the minimum applies to a mouse exactly as it does to a finger. A data grid earns its keep on density, though, so the grid grows the **hit area** rather than the **box**: small controls such as the tree, grouping, pivot, and master-detail toggles carry a transparent 24 px overlay centred on the icon, and handles such as the column resizer and the tool-panel splitter are absolutely positioned. Row heights, column widths, and indentation are identical to what they were before the target minimum existed.

A coarse pointer additionally enlarges the visible box of several controls. That is a comfort choice on top of the target minimum, not the thing that satisfies it.

You can override the token in your theme to increase target sizes for your deployment:
...
One control is sized separately. The [Shell](/grid/plugins/shell/) tool-panel splitter straddles the panel's docked edge so its target never covers the panel's own controls, which means it also has to widen the panel's clip region. Browsers reject a calculated `overflow-clip-margin`, so it reads its own token:

| Token | Default | Notes |
|---|---|---|
| `--tbw-tool-panel-resize-overhang` | `12px` | Half the splitter's target width. Raise it alongside `--tbw-touch-target-min`. |
