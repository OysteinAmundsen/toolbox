...
export class TaskListComponent {
  ...
  onGrid = effect(() => {
    ...
    grid.addEventListener('row-move', (e: CustomEvent) => {
      console.log('Row moved:', e.detail);
    });
  });
}
...
The drag handle can be placed at the end of each row instead of the start.

<Canvas of={RowReorderStories.HandleAtEnd} />

### Handle on Hover

Show the drag handle only when hovering over a row for a cleaner look.

<Canvas of={RowReorderStories.HandleOnHover} />
...
| `debounceMs`        | `number`                                                          | `300`     | Debounce time for rapid keyboard moves                            |
...
interface RowReorderConfig {
  ...
  debounceMs?: number;            // Debounce time for keyboard moves (default: 300)
