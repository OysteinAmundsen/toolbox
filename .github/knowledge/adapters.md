...
- DECIDED (Nov 2025, #237 follow-up): only the `CORE_CONSUMED_ADAPTER_METHODS` constant is exported from `public.ts`. The helper functions `assertAdapterConformance`/`reportAdapterConformance` stay internal — exporting them pushed `index.js` over the 45 kB gzipped budget. Each adapter spec inlines the 2-line filter; DRY is not worth the bundle cost.
