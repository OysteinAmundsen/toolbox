...
/**
 * Parity with core issue #438: the React adapter must accept nested dotted
 * paths in `field` by default AND support the strict `NestedPaths` opt-in
 * together with React JSX renderers.
 */
describe('nested dotted-path fields — React parity (#438)', () => {
  it('accepts nested paths by default (zero config)', () => {
    const columns: ColumnConfig<Deal>[] = [
      { field: 'deal.capture.field' },
      { field: 'deal.otherStuff.other', renderer: (ctx) => String(ctx.value) },
    ];
  ...
  it('supports strict NestedPaths opt-in with the TField generic', () => {
    const config: GridConfig<Deal, NestedPaths<Deal>> = {
      columns: [{ field: 'deal.capture.field', renderer: (ctx) => String(ctx.value) }],
    };
