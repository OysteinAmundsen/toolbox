...
<Canvas of={ExportStories.Default} />

### Export Selected Rows

When combined with SelectionPlugin, export only selected rows.

<Canvas of={ExportStories.ExportSelected} />
...
| --------------------- | -------------------------------------- | --------------------------- |
| `tbw-export-start`    | `{ format: string }`                   | Fired when export begins    |
| `tbw-export-complete` | `{ format: string, rowCount: number }` | Fired when export completes |
