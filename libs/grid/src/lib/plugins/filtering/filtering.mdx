...
| --- | --- | --- |
| `number` | **Range Slider** | Dual-thumb slider with min/max inputs |
| `date` | **Date Range Picker** | From/to date inputs with range selection |
| (other) | **Checkbox Set** | Standard multi-select with search |
...
call `suspendProcessing()` before the assignment:

```ts
data.splice(3, 0, newRow);
grid.suspendProcessing(); // skip filtering for this one update
grid.rows = data;
```

The flag auto-resets — the next `rows` assignment filters normally. See the
[API Reference → Suspend Row Processing](?path=/docs/grid-api--docs#suspend-row-processing)
for full details.

> **Tip:** Do **not** use `suspendProcessing()` for data refreshes (API responses,
> WebSocket updates). In those cases you want re-filtering to keep the grid consistent.
