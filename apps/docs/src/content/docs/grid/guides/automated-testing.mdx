...
|---|---|---|
| **CSS classes** ([`GridClasses`](/grid/api/core/variables/gridclasses/)) | `data-grid-row` (row container), `cell` (data cell), `header-row`, `header-cell`, `selected`, `editing`, `sorted-asc`, `sorted-desc`, `focused`, … | Semver-stable, `@since 0.1.1` |
| **Data attributes** ([`GridDataAttrs`](/grid/api/core/variables/griddataattrs/)) | `data-field="<col>"`, `data-row="<rowIndex>"`, `data-group-key` | Semver-stable |
| **CSS selectors** ([`GridSelectors`](/grid/api/core/variables/gridselectors/)) | Prebuilt selectors: `DATA_ROW`, `DATA_CELL`, `SELECTED_ROWS`, `EDITING_CELL`, `CELL_BY_FIELD(field)`, … | Semver-stable |
...
**If your app loads the [UMD bundle](/grid/getting-started/#option-1-cdn-no-build-step)** — `window.TbwGrid.queryGrid(...)` is already available, no setup needed. Substitute `window.TbwGrid.queryGrid` for `window.queryGrid` in every example below.
