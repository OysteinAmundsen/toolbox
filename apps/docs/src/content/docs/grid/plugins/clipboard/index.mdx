...
The Clipboard plugin brings familiar copy/paste functionality to your grid with full keyboard shortcut support (Ctrl+C, Ctrl+V). It handles single cells, multi-cell selections, and interoperates with Excel and other spreadsheet applications via tab-delimited output.
...
See [`ClipboardConfig`](./interfaces/clipboardconfig/) for the full list of options and defaults.

Two options are worth calling out:

- **`escapeFormulas`** — on by default. Values starting with `=`, `+`, `-`, `@`, tab or CR are
  prefixed with `'` so a spreadsheet treats them as text instead of evaluating them (CWE-1236).
- **`fillSelection`** — tiles the clipboard source across a larger multi-cell selection
  (see [Fill Selection](#fill-selection)).
...
With `selection: 'range'`, holding <kbd>Ctrl</kbd> while dragging adds disjoint ranges to the selection. Copying such a selection produces the **union bounding box** of all ranges, exactly as Excel does:

- cells inside at least one range carry their displayed value,
- cells inside the bounding box but outside every range are emitted as blanks.

Keeping the block rectangular means the paste lines up correctly in an external target such as Excel or a text editor.

Explicit bounds bypass this masking — `copy({ columns })` and `copyRows(indices)` always emit every cell of the requested columns/rows.
...
See [`CopyOptions`](./interfaces/copyoptions/) for every per-call option. `columns` and
`rowIndices` scope what is copied (row indices need not be contiguous); the remaining options
mirror [`ClipboardConfig`](./interfaces/clipboardconfig/) and fall back to its values when omitted.
