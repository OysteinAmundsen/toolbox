...
| `shell.toolPanel.closeOnClickOutside` | `boolean` | `false` | Close on outside click (ignored in `mode: 'push'`) |
...
If you need to replace the built-in toggle button entirely — for example, to use a design-system button (`<my-button>`, MUI `<Button>`, etc.) instead of styling `.tbw-toolbar-btn` — set `shell.header.toolPanelToggle: false`. The grid will not render the built-in button **or** the auto-inserted separator next to it; tool panels themselves remain fully functional and can be toggled from any element via [`grid.toggleToolPanel()`](./api-reference) (or `toggleToolPanelSection(id)` for a specific section).
