...
:::tip[Adapters auto-register the shell]
The React, Vue, and Angular wrappers register the `ShellPlugin` automatically when you use a
shell wrapper component (`GridToolPanel`, `GridToolButtons`, …) or pass `features.shell`, so
adapter users do not need the `import '@toolbox-web/grid/features/shell'` line.
:::
...
The shell used to be part of grid **core**: configured via a top-level `config.shell` field and
driven by `grid.register*` / `openToolPanel` element delegates. Those surfaces are **deprecated**
and will be **removed in v3.0.0**. Migrate as follows:

| Deprecated (v2.x, removed at v3) | Use instead |
|----------------------------------|-------------|
| _Implicit_ shell (no import) | `import '@toolbox-web/grid/features/shell'` |
| `config.shell = { … }` (top-level) | `config.features = { shell: { … } }` |
...
- In **v2.x** the shell is still auto-registered, so unmigrated code keeps working — the
  ...
  top-level `config.shell` field and `refreshShellHeader()` are merged/handled silently.
...
- **Adapters auto-register** the `ShellPlugin`, so React/Vue/Angular users need no change.
