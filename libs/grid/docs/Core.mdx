...
import * as HeaderRendererStories from './stories/header-renderers.stories';
import * as LoadingPlaygroundStories from './stories/loading-playground.stories';
import * as RTLStories from './stories/rtl.stories';
...
| Feature                    | Description                                          |
| -------------------------- | ---------------------------------------------------- |
| 🚀 **Virtualization**      | Handles 100k+ rows with smooth scrolling             |
| 🔧 **Framework Agnostic**  | Works in vanilla JS, React, Vue, Angular, Svelte     |
| ⌨️ **Keyboard Navigation** | Full arrow key, Tab, Enter, Escape support           |
| 🎨 **Themeable**           | CSS custom properties for complete customization     |
...
<template>
  <TbwGrid :rows="data" style="height: 400px">
...
Experiment with all core grid options using the controls below. [Open with controls →](?path=/story/grid-core--playground)
...
The grid implements [ARIA grid keyboard patterns](https://www.w3.org/WAI/ARIA/apg/patterns/grid/) out of the box—arrow keys, Tab, Enter, Escape, Home/End, Page Up/Down all work with no configuration required.
...
<Canvas of={RTLStories.Demo} layout="fullscreen" />
<Controls of={RTLStories.Demo} />

**Logical column pinning:** Use `pinned: 'start'` and `pinned: 'end'` instead of `'left'`/`'right'` for direction-independent pinning. See [PinnedColumnsPlugin](?path=/docs/grid-plugins-pinned-columns--docs) for details.
...
The grid also supports other light DOM elements like `<tbw-grid-header>` for shell configuration — see [Shell Components](#shell-components) below.
...
- [`GridConfig`](/docs/grid-api-core-interfaces-gridconfig--docs) — Grid-level options (fitMode, sortable, plugins, etc.)
- [`ColumnConfig`](/docs/grid-api-core-interfaces-columnconfig--docs) — Column-level options (field, header, width, renderer, etc.)
...
- **Percentages**: `0.156` → `15.6%`
- **Prefixes/suffixes**: `42` → `42 kg` or `#42`
...
**Style individual cells** based on their value — perfect for data validation, conditional formatting, or highlighting.
...
The renderer context ([`CellRenderContext`](/docs/grid-api-core-interfaces-cellrendercontext--docs)) provides:
- `value` — The cell's data value
- `row` — The full row data object
- `column` — The column configuration
- `field` — The field key

<Canvas of={GridStories.CustomRenderers} withToolbar />
...
**Preserving row position during insert/remove:**

When sorting or filtering is active, assigning `rows` normally re-applies those
pipelines — so an inserted row may jump to a different position. Call
`suspendProcessing()` before the assignment to keep it in place:

```typescript
data.splice(3, 0, newRow);
grid.suspendProcessing(); // skip sort/filter for this one update
grid.rows = data;
grid.animateRow(3, 'insert');
```

The flag auto-resets after one render cycle. See the [API Reference → Suspend Row Processing](?path=/docs/grid-api--docs#suspend-row-processing) for details.
...
- **`headerLabelRenderer`** — Customize just the label. Grid handles sort icons, filter buttons, and resize handles.
- **`headerRenderer`** — Full control over header content. Use `ctx.renderSortIcon()` and `ctx.renderFilterButton()` (with FilteringPlugin) to include standard elements.
...
By default, all rows share a fixed height (`28px`). For scenarios where rows need different heights—such as wrapped text, expandable content, or plugin-provided heights (like group headers)—the grid supports variable row heights with smart caching.

### Configuration

Use the `rowHeight` option in `gridConfig`:

```typescript
grid.gridConfig = {
  rowHeight: 40, // Fixed height for all rows (px)
};
```

For dynamic heights, pass a function that returns a **known height** or `undefined` to trigger auto-measurement:
...
grid.gridConfig = {
  rowHeight: (row, index) => {
    ...
    // Return undefined to let the grid measure this row's actual DOM height
    return undefined;
  },
};
```
...
When `rowHeight` returns `undefined`, the grid:

1. Renders the row to the DOM
2. Measures its actual height via `getBoundingClientRect()`
3. Caches the result for future scroll position calculations

This enables rows with dynamic content (multi-line text, images, etc.) to render at their natural height.

### Row Identity for Height Caching

The grid caches measured row heights to avoid repeated DOM measurements. Cache keys are determined by:

| Row Type | Cache Key | When to Use |
|----------|-----------|-------------|
| Object rows | WeakMap (by reference) | Default for object data |
| Primitive rows | `rowId` config option | Required when rows are primitives or recreated |
| Plugin rows | `__rowCacheKey` property | Auto-set by plugins (e.g., `'group:Engineering'`) |

**Important:** If rows are recreated (e.g., from JSON parsing), configure `rowId` so the cache persists across updates:

```typescript
grid.gridConfig = {
  rowId: 'id', // Use the 'id' field as unique identifier
  rowHeight: (row) => row.expanded ? undefined : 32, // Measure expanded rows
};
```

### Plugin-Provided Heights

Plugins can implement `getRowHeight(row, index)` to provide known heights for their synthetic rows:

- **GroupingRowsPlugin**: Use `groupRowHeight` config for group header rows
- **MasterDetailPlugin**: Returns configured detail panel height
- **TreePlugin**: Can specify different heights for parent vs. leaf nodes
...
grid.gridConfig = {
  ...
  rowHeight: (row) => {
    // Plugin handles group rows automatically via getRowHeight hook
    // This callback is for data rows only
    return row.hasNotes ? undefined : 28;
  },
};
```
...
|----------|-------------|----------|
| Fixed `number` | ⚡ Fastest | Uniform row heights |
| Function returning numbers | ⚡ Fast | Known variable heights (e.g., by row type) |
| Function returning `undefined` | 🐢 Slower | Dynamic content requiring DOM measurement |

**Best practices:**
- Return known heights whenever possible to avoid DOM measurement
- Use `rowId` config when rows may be recreated between renders
- Plugins like `GroupingRowsPlugin` handle their own heights—you don't need to account for them in `rowHeight`
...
| `cell-change`         | [`CellChangeDetail`](/docs/grid-api-core-interfaces-cellchangedetail--docs) | Row updated via Row Update API                  |
...
- **[API Reference](?path=/docs/grid-api--docs)** — Complete properties, methods, events, and CSS variables
- **[Theming](?path=/docs/grid-theming--docs)** — Customize colors, fonts, and sizing with CSS variables
- **[Plugins](?path=/docs/grid-plugins--docs)** — Extend the grid with selection, editing, filtering, and more
- **[Performance Guide](?path=/docs/grid-dev-performance--docs)** — Optimization tips for large datasets
- **[Accessibility](?path=/docs/grid-dev-accessibility--docs)** — ARIA roles, keyboard navigation, and screen reader support
