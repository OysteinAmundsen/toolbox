...
- DECIDED (#449, SC 2.5.7): plain `click` on a field chip opens `plugins/shared/drag-alternative-menu` (HTML5 DnD fires `dragstart` only on a real drag ⇒ click == tap). (a) `role`/`aria-label`/`tabindex=-1` sit on `.tbw-pivot-chip-label`, NOT the chip — `role="button"` makes descendants presentational and would hide `.tbw-pivot-chip-remove`; available-field chips are childless so the role sits on the chip. (b) `chipMenu` is a MODULE-LEVEL singleton: `refreshPanel()` does `innerHTML=''` + re-render without running cleanup, so a per-chip menu would be torn out mid-click; `menu.open()` re-parents to the anchor's grid host ⇒ safe across grids; `renderPivotPanel` cleanup disposes+nulls it. (c) `onReorderFieldInZone` takes an INSERT-BEFORE index in the pre-move list ⇒ up = `index-1`, down = `index+2`. Owner: `pivot-panel.ts`. Tests: `pivot-panel.spec.ts` › "click-only chip menu (SC 2.5.7)".
