...
**Logical column pinning:** Use `sticky: 'start'` and `sticky: 'end'` instead of `'left'`/`'right'` for direction-independent pinning. See [PinnedColumnsPlugin](?path=/docs/grid-plugins-pinned-columns--docs) for details.
