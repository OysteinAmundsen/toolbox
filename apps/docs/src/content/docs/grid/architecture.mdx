...
gridConfig: {
  shell: {
    ...
    toolPanel: {
      ...
      defaultOpen: 'filters',      // Section auto-expanded on first open (also opens the sidebar today — see #259)
...
Internally, plugins communicate through two mechanisms described under [Plugin Development → Plugin Communication](/grid/plugin-development/architecture/#plugin-communication): the **Event Bus** (async, fire-and-forget) and the **Query System** (sync, request-response). Public `CustomEvent`s are dispatched by the core grid or by individual plugins after their internal processing completes.
