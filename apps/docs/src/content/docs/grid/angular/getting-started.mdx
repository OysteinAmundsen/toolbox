...
accepts an RxJS `Observable` directly — no wrapper needed. The grid subscribes once
and calls `unsubscribe()` when a request is superseded (sort change, fast scrolling),
which is what triggers `HttpClient` to cancel the underlying XHR. No stale buffer,
no manual `firstValueFrom`.
