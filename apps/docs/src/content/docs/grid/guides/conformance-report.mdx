...
The **Verified by** column names the evidence, and the evidence is one of four kinds. They are not
equally strong, and the report does not pretend otherwise:

- **Automated, in CI** — a unit, integration or Playwright test that fails the build on regression.
  This covers axe-core rule scans on all four framework adapters, computed-style measurement of
  focus indicators, contrast and target size, and ARIA-tree snapshots that pin the exposed
  semantics of the grid skeleton and its transient surfaces.
- **Computed-ratio review** — colour pairs measured in a browser. Where a criterion says so, the
  measurement is asserted in CI; where it does not, it was measured once at the report date.
- **Manual review** — a criterion for which no automated check is meaningful (nothing flashes, no
  instruction depends on shape). Re-checked when the relevant code changes.
- **Author responsibility** — the grid supplies a conforming mechanism; your content can still
  break it.

**No assistive technology is driven in CI.** Announcements depend on the AT, the browser and the
user's verbosity settings, none of which a headless browser models — so a green axe run is evidence
that the semantics are correct, not that a screen reader says something useful. Screen-reader
verification is a manual procedure run against release candidates, using the
[manual verification checklist](/grid/guides/accessibility/#manual-verification-checklist). Where a
criterion's claim rests on what a user *hears* rather than on what is in the DOM — 1.3.1, 3.3.1,
3.3.2, 4.1.2, 4.1.3 in particular — read the automated evidence as necessary but not sufficient.
...
| 4.1.2 Name, Role, Value | Supports | Verified by the axe-core scan on all four framework demos, plus targeted tests for sort state, expansion state, selection state and editor naming. Transient surfaces — filter panel, context menu and submenu, overlay editor, export dialog, expanded detail and tree rows, open tool panel — are scanned *after* the interaction that creates them, and the exposed accessibility tree is pinned by snapshot so a silent naming regression fails the build. | [accessibility.spec.ts](https://github.com/OysteinAmundsen/toolbox/blob/main/e2e/tests/accessibility.spec.ts) — "no WCAG 2.2 AA violations axe can detect"; [a11y.spec.ts](https://github.com/OysteinAmundsen/toolbox/blob/main/apps/docs-e2e/tests/a11y.spec.ts) — transient surfaces; [aria-snapshot.spec.ts](https://github.com/OysteinAmundsen/toolbox/blob/main/apps/docs-e2e/tests/aria-snapshot.spec.ts); [editor-injection.spec.ts](https://github.com/OysteinAmundsen/toolbox/blob/main/libs/grid/src/lib/plugins/editing/internal/editor-injection.spec.ts) |
...
| 1.4.3 Contrast (Minimum) | Supports | All six shipped themes meet 4.5:1 on every text pair in both light and dark mode; `dg-theme-contrast.css` / `dg-theme-large.css` reach the Level AAA 7:1. Translucent row tints are composited over the grid background before scoring, so the ratio is the one actually painted. **Author responsibility:** a theme of your own is yours to verify. | [accessibility.spec.ts](https://github.com/OysteinAmundsen/toolbox/blob/main/e2e/tests/accessibility.spec.ts) — axe `color-contrast` runs against the shipped default theme; [theme-contrast.spec.ts](https://github.com/OysteinAmundsen/toolbox/blob/main/apps/docs-e2e/tests/theme-contrast.spec.ts) asserts the ratio for every theme × both modes in a browser on every run |
...
| 2.5.8 Target Size (Minimum) | Supports | Every control answers a pointer across at least 24 × 24 px, on a mouse as well as a finger. Small controls reach it with a transparent overlay, so visual density is unchanged. Raise `--tbw-touch-target-min` to 44 px for the Level AAA criterion. | [accessibility.spec.ts](https://github.com/OysteinAmundsen/toolbox/blob/main/e2e/tests/accessibility.spec.ts) — "small grid controls answer a pointer across a 24px square", "the tool panel splitter answers a pointer across a 24px square"; [a11y.spec.ts](https://github.com/OysteinAmundsen/toolbox/blob/main/apps/docs-e2e/tests/a11y.spec.ts) measures in-cell controls too, discounting only the part of a control a scroller actually clips |
...
- **`e2e/tests/accessibility.spec.ts`** — the axe-core scan under the WCAG 2.2 AA ruleset, run
  ...
  criterion states it.
- **`apps/docs-e2e/tests/a11y.spec.ts`** — the same ruleset, but scanning the **whole document** of
  six documentation pages rather than just the `<tbw-grid>` subtree, and with none of the structural
  rules suppressed. It is the check that catches role-tree defects a grid-scoped scan cannot see —
  an overlay promoted into `role="grid"`, a card row without a cell role, an unnamed checkbox in a
  custom renderer. It also carries the SC 2.5.8 measurement, which models the criterion's
  *effective target* and *spacing* exceptions instead of relying on axe's `target-size`.
