...
When using `FormArray` with `FormGroup`s, you can access row-level validation state via [`getFormArrayContext()`](/grid/angular/api/utilities/getFormArrayContext/). The returned [`FormArrayContext`](/grid/angular/api/types/FormArrayContext/) exposes:
