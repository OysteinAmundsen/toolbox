...
// test suite — other adapter tests (and `<DataGrid>` itself) depend on the
// registry being fully populated.
...
describe('feature-prop-keys built-in pre-registration', () => {
  it('pre-registers all React built-in feature prop names at module load', () => {
    // Reload would re-execute module; instead, snapshot what was registered on
    // first import (captured above before the clear-each-test rig kicks in).
    // This asserts the surface contract from gh #356 §7: registry pre-populates
    // from core, side-effect imports augment for third-party.
