...
- DECIDED (Aug 2026, #453 — COPY = union bbox of ALL ranges + gap masking): `#resolveData` MUST derive row AND column bounds from min/max over **every** `selection.ranges` (it used `ranges[ranges.length-1]`, so a CTRL+drag multi-range selection copied only the last range). It also returns `contains(rowOffset,colOffset)` — non-null ONLY when both axes came from the selection — and `#buildDisplayGrid` emits `''` for cells outside every range (Excel-style rectangular multi-range copy). Mask applies to `text/plain`, the `text/html` table AND `rawRows` (else a same-app paste resurrects unselected values). MUST be skipped when `options.columns` / `options.rowIndices` are supplied, else `copyRows()` blanks everything. Offsets map to absolute indices via captured `rowIndices`/`colIndices` arrays — `resolveColumns()` drops hidden/utility columns from the slice, so arithmetic `minCol + offset` is WRONG.
- INVARIANT (row mode ≠ range mode on copy): in `mode: 'row'` the selection query merges Ctrl+clicked indices into SEVERAL whole-row ranges (`SelectionPlugin` `toSelectionResult`, rows sorted then contiguous runs merged, each `col 0..colCount-1`). Columns are NOT selection-derived there, so `contains` is null and no mask exists — `#resolveData` MUST therefore DROP rows in the `minRow..maxRow` span that no range covers. Blanking them (range-mode behavior) or copying them (naive bbox) both leak/garble unselected rows.
