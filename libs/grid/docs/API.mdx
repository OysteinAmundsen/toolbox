...
| -------------------- | ------------------- | ---------- | ----------------------------------- |
| `cell-commit`        | `CellCommitDetail`  | ✅ Yes     | Cell value was edited (inline)      |
...
| `cell-activate`       | `CellActivateDetail`   | ✅ Yes     | Cell activated via Enter or click. Check `trigger` |
| `activate-cell`       | `ActivateCellDetail`   | ✅ Yes     | ⚠️ **Deprecated.** Use `cell-activate` instead     |
...
interface ColumnConfig {
  ...
  headerLabelRenderer?: (ctx: HeaderCellContext) => string | HTMLElement;
...
Customize how column headers are displayed with two levels of control:

**`headerLabelRenderer`** - Customize only the label content. The grid automatically adds sort icons and resize handles.
...
const columns: ColumnConfig[] = [
  {
    field: 'name',
    header: 'Name',
    headerLabelRenderer: ({ value }) => `${value} *`, // Add asterisk to label
  },
  {
    field: 'required',
    header: 'Required',
    headerLabelRenderer: ({ value, column }) => {
...
**`headerRenderer`** - Full control over the entire header cell. You receive helper functions to optionally include sort icons and resize handles.
...
| `cellEl` | `HTMLElement` | The header cell element |
| `renderSortIcon()` | `() => HTMLElement \| null` | Creates sort indicator (null if not sortable) |
| `renderResizeHandle()` | `() => HTMLElement` | Creates resize handle element |

See the [Header Renderers](?path=/docs/grid-header-renderers--docs) stories for live demos.
