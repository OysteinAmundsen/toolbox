...
{/* Regenerate with: bun nx typedoc grid-react */}
...
function processModule(module: TypeDocNode, outDir: string): void {
  ...
  if (components.length) {
    ...
    for (const node of components) {
      ...
      const outPath = join(outDir, 'Components', `${node.name}.mdx`);
      mkdirSync(join(outDir, 'Components'), { recursive: true });
      writeFileSync(outPath, mdx);
      console.log(`    ✓ Components/${node.name}.mdx`);
    }
  }
  ...
  if (hooks.length) {
    ...
    for (const node of hooks) {
      ...
      const mdx = genFunction(node, title);
      const outPath = join(outDir, 'Hooks', `${node.name}.mdx`);
      mkdirSync(join(outDir, 'Hooks'), { recursive: true });
      writeFileSync(outPath, mdx);
      console.log(`    ✓ Hooks/${node.name}.mdx`);
    }
  }
