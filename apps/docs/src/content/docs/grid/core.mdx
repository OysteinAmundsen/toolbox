...
| `shell.toolPanel.defaultOpen` | `string` | — | Accordion section to auto-expand on first open. **Deprecated:** in v2.x this also opens the sidebar; in v3.0.0 it will only pre-select the section (see [#259](https://github.com/OysteinAmundsen/toolbox/issues/259)). Combine with `initialState: 'open'` for forward-compatible behavior. |
| `shell.toolPanel.initialState` | `'open' \| 'closed'` | `'closed'` | Whether the sidebar starts open on grid load. Overrides the legacy `defaultOpen` open-on-load behavior when set. |
| `shell.toolPanel.locked` | `boolean` | `false` | Lock the sidebar open. Implies `initialState: 'open'`, makes `closeToolPanel()` a no-op, and hides the built-in toggle button. |
| `shell.toolPanel.persistState` | `boolean` | `false` | Remember open/closed state |
| `shell.toolPanel.closeOnClickOutside` | `boolean` | `false` | Close on outside click (ignored in `mode: 'push'` or when `locked: true`) |
| `shell.toolPanel.mode` | `'overlay' \| 'push'` | `'overlay'` | `'overlay'` floats over the grid; `'push'` reflows the grid sideways so all cells stay visible |
...
If you need to replace the built-in toggle button entirely — for example, to use a design-system button instead of styling `.tbw-toolbar-btn` — set `shell.header.toolPanelToggle: false`. The grid will not render the built-in button **or** the auto-inserted separator next to it; tool panels themselves remain fully functional and can be toggled from any element via [`grid.toggleToolPanel()`](./api-reference), opened directly on a specific section via [`grid.openToolPanel(panelId)`](./api-reference), or driven section-by-section with `toggleToolPanelSection(id)`.
      ...
      toolbarContents: [
        {
          ...
          render: (container) => {
            ...
            filters.addEventListener('click', () => grid.openToolPanel('filters'));

            const settings = document.createElement('my-button');
            settings.textContent = 'Settings';
            settings.addEventListener('click', () => grid.openToolPanel('settings'));
...
Passing a `panelId` to `openToolPanel()` jumps straight to that accordion section (one click, no double-tap). It takes precedence over `shell.toolPanel.defaultOpen`; if the ID isn't registered, the call falls back to default behavior and emits a `TBW072` warning.

---

### Hide the Shell Header Bar Entirely

`toolPanelToggle: false` removes the built-in toggle button but keeps the header bar (with its title and any toolbar contents). To suppress the **entire** `.tbw-shell-header` bar — for example when you want to open tool panels from a custom column header icon and own all chrome yourself — set `shell.header.visible: false`.
  ...
  columns: [
    // …
    {
      ...
      headerRenderer: () => {
        ...
        btn.addEventListener('click', () => grid.toggleToolPanel());
        return btn;
      },
    },
  ],
...
When `header.visible: false`, tool panels remain fully functional and are opened with [`grid.toggleToolPanel()`](./api-reference) / [`grid.openToolPanel(panelId)`](./api-reference) from any element you like. Because there is no header toggle to close the panel, the grid provides three additional dismissal affordances:

- **Close button** — a small `✕` button is rendered in the top-right of the panel (exposed as the `tool-panel-header` and `tool-panel-close` CSS parts). It is omitted when `shell.toolPanel.locked` is `true`, since a locked panel cannot be closed.
...
- **Click-outside** — set `shell.toolPanel.closeOnClickOutside: true` to dismiss an open overlay panel when clicking anywhere in the window outside the panel. This is opt-in and, like Esc, is a no-op in `push` mode.
