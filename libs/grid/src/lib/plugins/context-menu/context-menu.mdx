...
<Canvas of={ContextMenuStories.WithSubmenus} />

### Conditional Items

Show/hide items based on context.

<Canvas of={ContextMenuStories.ConditionalItems} />
...
| `showDefaultItems` | `boolean`    | `true`  | Include default Copy/Paste items |
...
| ----------------- | ------------------------------- | ----------------- |
| `tbw-menu-open`   | `{ rowIndex, colIndex, items }` | Menu opened       |
| `tbw-menu-close`  | `{}`                            | Menu closed       |
| `tbw-menu-action` | `{ id, context }`               | Menu item clicked |
