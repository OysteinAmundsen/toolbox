...
For large hierarchical datasets, use a **data source** to load top-level nodes on demand as the user scrolls. Each page of top-level nodes is fetched with its children already embedded — the grid handles expand/collapse locally while pagination operates at the top-level node granularity.
...
const dataSource = {
  async getRows(params) {
    // params: { startNode, count, sortModel, filterModel }
    ...
    return {
      ...
      totalTopLevelCount: data.total,  // Total top-level nodes on server
    };
  },
};
...
function LazyTree() {
  return (
    <DataGrid
      ...
      tree={{ childrenField: 'children', dataSource, pageSize: 50 }}
    />
  );
}
```
...
<template>
  <TbwGrid :tree="{ childrenField: 'children', dataSource, pageSize: 50 }">
    <TbwGridColumn field="name" header="Name" />
...
@Component({
  ...
  imports: [Grid],
  template: `
    <tbw-grid [columns]="columns" [tree]="treeConfig" style="height: 400px; display: block;" />
  `,
})
...
When using a `dataSource`, the following additional methods are available:

```ts
plugin.setDataSource(dataSource);        // Replace the data source at runtime
plugin.refreshDataSource();              // Clear loaded data and re-fetch from start
plugin.loadMore();                       // Manually trigger loading the next page
plugin.isLoading;                        // true while a fetch is in progress
plugin.getLoadedTopLevelCount();         // Number of top-level nodes loaded so far
plugin.getTotalTopLevelCount();          // Total reported by the server (-1 if unknown)
```
...
When using a `dataSource`, the following events track fetch lifecycle:

```ts
grid.on('tree-load-start', () => console.log('Fetching next page…'));
grid.on('tree-load-end', () => console.log('Page loaded'));
grid.on('tree-load-error', (e) => console.error('Fetch failed', e.detail));
```

| Event | Fired when |
|---|---|
| `tree-load-start` | A page fetch begins |
| `tree-load-end` | A page fetch succeeds |
| `tree-load-error` | A page fetch fails |
