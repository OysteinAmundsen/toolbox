...
- DECIDED (#326, May 2026): browsers cap a single element's rendered height at ~33.5M px (Chromium `2^25`); above the cap the faux-vscroll spacer silently truncates (with `rowHeight: 34`, hit at ~986,895 rows), making the dataset's tail unreachable via native scrollbar / `Ctrl+End` / `scrollToRow(N)`. Fix: clamp spacer at `MAX_ELEMENT_HEIGHT_PX` (33_500_000) and store a `ScrollMapping` (`rawContentHeight`, `spacerHeight`, `viewportHeight`, `capped`) on `VirtualState`. Three call sites translate: `refreshVirtualWindow` + `grid.ts` scroll listener → `toVirtualScrollTop`; `focus-manager.scrollToRow` → `fromVirtualScrollTop`. Identity below the cap. Public surface re-exported from [public.ts](libs/grid/src/public.ts): `MAX_ELEMENT_HEIGHT_PX`, `ScrollMapping`, `computeScrollMapping`, `toVirtualScrollTop`, `fromVirtualScrollTop`. `tbw-scroll` event + plugin `onScroll` continue to report **native** spacer-space coords (self-consistent with `fauxScrollbar.scrollTop`). Files: [virtualization.ts](libs/grid/src/lib/core/internal/virtualization.ts), [virtualization-manager.ts](libs/grid/src/lib/core/internal/virtualization-manager.ts), [focus-manager.ts](libs/grid/src/lib/core/internal/focus-manager.ts), [grid.ts](libs/grid/src/lib/core/grid.ts) scroll listener. Tests: `virtualization.spec.ts > Scroll Mapping Tests` + `__tests__/integration/scroll-mapping-cap.spec.ts`. RULED OUT: pure fractional mapping for the sub-cap case (loses pixel-accurate wheel/keyboard granularity).
- TENSION (#326): above the cap, 1 spacer-px of `scrollTop` advances many rows of content, so wheel/keyboard ticks visibly skip rows. Sub-cap behavior unchanged. A future anchor-offset model (track `firstVisibleRowIndex` separately and adjust `scrollTop` to keep the fraction in sync) would smooth this; not implemented because the basic algorithm already restores reachability of every row.
