...
## shared-store (cross-module-instance identity registry)

- OWNS: page-wide singleton bag of identities and registries, anchored on `globalThis[Symbol.for('@toolbox-web/grid/shared')]`. Buckets: `features: Map<string, RegistryEntry>` (feature factories), `featureResolver?: (features) => Plugin[]` (set by `setFeatureResolver`), `reactContexts: Record<string, unknown>`, `vueKeys: Record<string, symbol>`, `ngTokens: Record<string, unknown>`. Files: [shared-store.ts](libs/grid/src/lib/core/internal/shared-store.ts), [feature-hook.ts](libs/grid/src/lib/core/internal/feature-hook.ts), [registry.ts](libs/grid/src/lib/features/registry.ts).
- INVARIANT: `getOrCreateShared(bucket, key, factory)` is first-wins per `(bucket, key)`. The factory only runs on first call; later callers (including a second bundled copy of `@toolbox-web/grid`) get the same instance. `featureRegistry` and `getFeatureResolver()` read through the same store, so a feature registered in one copy is visible to all copies on the page.
- INVARIANT: `customElements.define('tbw-grid', …)` AND `globalThis.DataGridElement = DataGridElement` are both first-wins guards. The element class belongs to whichever copy mounted first; later copies see the existing class via `globalThis.DataGridElement` and skip redefinition. `DataGridElement.shared` exposes the store for debugging.
- DECIDED (May 2026, #338): two bundled copies of `@toolbox-web/grid` (or any `-react`/`-vue`/`-angular` adapter) on one page used to mint independent React Contexts / Vue InjectionKeys / Angular InjectionTokens / feature registries. Symptoms: React crashed at the ContextConsumer (`TypeError: g is not a function`), Vue silently fell back to `inject(KEY, ref(null))` defaults (features no-op), Angular threw `NullInjectorError` or returned `null` with `{ optional: true }`. Fix: every per-page identity primitive is created via `getOrCreateShared(bucket, key, factory)` so all copies converge on one instance. WHY a globalThis symbol slot (not a module-level singleton): `Symbol.for(...)` survives `vi.resetModules()` and bundler-induced module duplication; module-level state does not. RULED OUT: shipping a "marker package" peer dep — does not survive a strict version-pinning consumer that bundles each adapter against its own grid copy.
- DECIDED (May 2026, #338): adapters import `getOrCreateShared` from `@toolbox-web/grid` (re-exported via [public.ts](libs/grid/src/public.ts), `@internal` JSDoc). Pulling that import is the only adapter-side requirement — every Context / Symbol / InjectionToken declaration uses it. Tests in `shared-store.spec.ts`, `shared-context.spec.ts` (react), `shared-injection-keys.spec.ts` (vue), `shared-injection-tokens.spec.ts` (angular) prove identity convergence using `vi.resetModules()` + dynamic re-import to simulate the dual-copy scenario.
