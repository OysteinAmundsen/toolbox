...
When a cell renderer or group header renderer returns an **HTML string**, the grid runs it through an internal sanitizer that strips `<script>` elements and event-handler attributes (`onclick`, `onerror`, etc.) before inserting it into the DOM. This is defense-in-depth — always validate your data upstream, but accidental injection through these renderer paths will not execute. Renderers that return `HTMLElement` / `Node` / framework components are inserted as-is and are not sanitized.
:::
...
If styles aren't applying in production, see [Styles not applying (CSP) in Troubleshooting](/grid/guides/troubleshooting/#styles-not-applying-csp) for diagnosis steps.
...
**Target sizes:** Core is ≤170 kB raw (≤50 kB gzipped, soft warning at 45 kB). Each plugin adds 2–15 kB.
...
If you hit scroll stuttering or blank rows at runtime, see [Performance](/grid/guides/troubleshooting/#performance) and [Height & Virtualization](/grid/guides/troubleshooting/#height--virtualization) in Troubleshooting.
...
- **Wait for grid readiness** — The grid renders asynchronously. Wait for `.data-grid-row` elements before asserting.
...
grid.on('sort-change', ({ field, direction }) => {
  analytics.track('grid_sort', { field, direction });
});

grid.on('filter-change', ({ filters, filteredRowCount }) => {
  analytics.track('grid_filter', { filters, filteredRowCount });
});
```
