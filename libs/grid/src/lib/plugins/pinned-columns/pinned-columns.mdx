...
The Pinned Columns plugin freezes columns to the left or right edge of the grid—essential for keeping key identifiers or action buttons visible while scrolling through wide datasets. Just set `sticky: 'left'` or `sticky: 'right'` on your column definitions and the plugin handles the rest.
...
grid.gridConfig = {
  columns: [
    { field: 'id', header: 'ID', sticky: 'left', width: 80 },
    { field: 'name', header: 'Name' },
    { field: 'email', header: 'Email' },
    { field: 'department', header: 'Department' },
    { field: 'salary', header: 'Salary', type: 'currency' },
    { field: 'actions', header: 'Actions', sticky: 'right', width: 120 },
  ],
...
> **Note:** To pin or unpin columns at runtime, update the column's `sticky` property in your `gridConfig.columns` and re-assign the config. The plugin reads `sticky` from column definitions during each render cycle.
