      ...
      dataSource: {
        getRows: async ({ startRow, endRow, sortModel }) => {
          ...
          const response = await fetch(
            `/api/data?from=${startRow}&to=${endRow}&sort=${sortQuery}`,
          );
...
When you only need to override sort for **one column** (e.g. a single field
that should sort server-side, while the rest sort client-side), `sortComparator`
can do that too — it can fire its own fetch and return a Promise-aware proxy.
For most use cases though, `ServerSidePlugin` is the cleaner fit.
