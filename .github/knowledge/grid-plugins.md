...
- GOTCHA (cell-commit → updateRow re-entrancy): because `updateRow` now re-emits `cell-commit`, a consumer that both LISTENS to `cell-commit` AND calls `grid.updateRow()` from inside that listener (e.g. to apply field cascades) will recurse infinitely (`RangeError: Maximum call stack size exceeded`). The consumer MUST guard its handler against re-entrancy (a boolean flag wrapping the cascade `updateRow`) — the cascade is already complete on the first pass. Seen Jul 2026 in cargo-tracker capture2 `onCellCommit`. The `cell-commit` event does NOT currently carry the `UpdateSource`, so a re-entrancy flag (not a source check) is the available guard.
