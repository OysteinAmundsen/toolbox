...
The `@toolbox-web/grid-angular` package integrates with Angular Reactive Forms, allowing the grid to act as a form-bound component with cell-level validation.

> **Pair with the editing directive.** [`GridFormArray`](/grid/angular/api/directives/gridformarray/) and [`GridLazyForm`](/grid/angular/api/directives/gridlazyform/) are themselves form-binding directives — they do **not** turn editing on. To allow cell edits, also add [`GridEditingDirective`](/grid/angular/api/directives/grideditingdirective/) (from `@toolbox-web/grid-angular/features/editing`) to the component's `imports`, **or** configure editing through `[gridConfig]="{ features: { editing: … } }"`.
...
> **Directive:** [`GridFormArray`](/grid/angular/api/directives/gridformarray/) · binds via the `[formArray]` input on `<tbw-grid>`.
...
When using `FormArray` with `FormGroup`s, you can access row-level validation state via `getFormArrayContext()` (re-exported from [`GridFormArray`](/grid/angular/api/directives/gridformarray/)). The returned [`FormArrayContext`](/grid/angular/api/types/formarraycontext/) exposes:
...
`*tbwEditor` exposes the core [`ColumnEditorContext`](/grid/api/core/interfaces/columneditorcontext/)
— `value`, `row`, `field`, `column`, `rowId`, `updateRow` and `onValueChange` — under the Angular
names documented in [`GridEditorContext`](/grid/angular/api/types/grideditorcontext/), where the
commit and cancel callbacks are `onCommit` / `onCancel`.

Two properties exist only in the Angular adapter:

| Property    | Type | Description |
|-------------|------|-------------|
| `$implicit` | `TValue` | Cell value, bound by `*tbwEditor="let value"` |
| `control`   | [`AbstractControl`](https://angular.dev/api/forms/AbstractControl) \| `undefined` | Cell `FormControl`, present only when bound to a `FormArray` + `FormGroup` |

> **Auto-wiring:** the adapter listens for native `commit` / `cancel` `CustomEvent`s on the rendered component. If your editor dispatches them (or extends [`BaseGridEditor`](/grid/angular/base-classes/) which does), you can omit the `(commit)="onCommit($event)"` binding entirely.
...
This integrates with the EditingPlugin's validation system, meaning Angular's `Validators.required`, `Validators.email`, etc. automatically show visual feedback in the grid.
...
> **Directive:** [`GridLazyForm`](/grid/angular/api/directives/gridlazyform/) · [`LazyFormFactory`](/grid/angular/api/types/lazyformfactory/) · [`RowFormChangeEvent`](/grid/angular/api/types/rowformchangeevent/)
...
The `(rowFormChange)` output emits a [`RowFormChangeEvent`](/grid/angular/api/types/rowformchangeevent/) every time the row's `FormGroup` value or status changes. The event includes `rowIndex`, `rowId?`, `row`, `formGroup`, `values`, `valid`, and `dirty`.

```typescript
import { Component } from '@angular/core';
import { Grid } from '@toolbox-web/grid-angular';
import type { RowFormChangeEvent } from '@toolbox-web/grid-angular/features/editing';
