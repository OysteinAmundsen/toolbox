...
- DECIDED (#326, May 2026): browsers cap a single element's rendered height at ~33.5M px (Chromium `2^25`); above the cap the faux-vscroll spacer silently truncates (with `rowHeight: 34`, hit at ~986,895 rows), making the dataset's tail unreachable via native scrollbar / `Ctrl+End` / `scrollToRow(N)`. Fix: clamp spacer at `MAX_ELEMENT_HEIGHT_PX` (33_500_000) and store a `ScrollMapping` (`rawContentHeight`, `spacerHeight`, `viewportHeight`, `capped`) on `VirtualState`. Four call sites translate: `refreshVirtualWindow` + `grid.ts` scroll listener → `toVirtualScrollTop`; `focus-manager.scrollToRow` + `keyboard.ensureCellVisible` → `fromVirtualScrollTop` (the latter compares `rowIndex × rowHeight` (virtual) against `scrollTop` (native) — both must be in the same space, and the write back is in native). Identity below the cap. Public surface re-exported from [public.ts](libs/grid/src/public.ts): `MAX_ELEMENT_HEIGHT_PX`, `ScrollMapping`, `computeScrollMapping`, `toVirtualScrollTop`, `fromVirtualScrollTop`. `tbw-scroll` event + plugin `onScroll` continue to report **native** spacer-space coords (self-consistent with `fauxScrollbar.scrollTop`). Files: [virtualization.ts](libs/grid/src/lib/core/internal/virtualization.ts), [virtualization-manager.ts](libs/grid/src/lib/core/internal/virtualization-manager.ts), [focus-manager.ts](libs/grid/src/lib/core/internal/focus-manager.ts), [keyboard.ts](libs/grid/src/lib/core/internal/keyboard.ts), [grid.ts](libs/grid/src/lib/core/grid.ts) scroll listener. Tests: `virtualization.spec.ts > Scroll Mapping Tests` + `keyboard.spec.ts > above MAX_ELEMENT_HEIGHT_PX (fractional scroll mapping)` + `__tests__/integration/scroll-mapping-cap.spec.ts`. RULED OUT: pure fractional mapping for the sub-cap case (loses pixel-accurate wheel/keyboard granularity).
- TENSION (#326): above the cap, 1 spacer-px of `scrollTop` advances many rows of content, so direct **scroll input** (wheel notch, scrollbar drag, Space) visibly skips rows. Per-row keyboard nav (Arrow/Tab/PageUp/Down) is unaffected — focus moves by row index and `ensureCellVisible` translates through the mapping. Sub-cap behavior unchanged. A future anchor-offset model (track `firstVisibleRowIndex` separately and adjust `scrollTop` to keep the fraction in sync) would smooth wheel/scrollbar; not implemented because the basic algorithm already restores reachability of every row.
...
- DECIDED (May 2026, #321): empty-state overlay (`emptyRenderer`) is mutually exclusive with the loading overlay — `loading=true` always wins, even with zero rows, to avoid flashing "No data" during the initial fetch. The overlay element is **recreated on every show** (no DOM caching) so renderer changes reflect immediately. Evaluation runs in two places: (a) `set loading()` after `#updateLoadingOverlay()`, and (b) `_schedulerAfterRender()` after `pluginManager.afterRender()` so plugins that mutate the rendered row count (filtering, grouping) trigger empty-state recompute. `EmptyContext.filteredOut` is `sourceRowCount > 0 && renderedRowCount === 0`. Default mount target is `.rows-container` (`emptyOverlay: 'rows'`); set `'grid'` to cover the header too. `emptyRenderer: null` is the documented opt-out (suppresses the built-in default). Files: [empty.ts](libs/grid/src/lib/core/internal/empty.ts), [empty.css](libs/grid/src/lib/core/styles/empty.css), tests in `empty.spec.ts` + `__tests__/integration/empty-state.spec.ts`.

- DECIDED (May 2026, #338 / #340 reverted): **do NOT introduce a `globalThis`-anchored cross-bundle shared store** (feature registry, React Context, Vue `InjectionKey`, Angular `InjectionToken`). PR #340 shipped and was reverted. WHY: each bundle owns its **own** React/Vue/Angular runtime (Equinor Roma `<AppLoader>`, Module Federation without `shared:`, single-spa without shared deps); cross-bundle Context object identity is a mismatch — bundle B's reconciler receives a Context whose `$$typeof` was minted by bundle A's React → "Invalid hook call" or silent `===` failure. The `features` bucket separately risks v2.13 factory wired to B's class internals → SPA-session crash. RULED OUT: (a) versioning the bucket keys (still shares Context minted by wrong React); (b) opt-in shared store (silent failure → nobody opts in until prod crash). Files removed in revert: `libs/grid/src/lib/core/internal/shared-store.ts` + specs, adapter `shared-*` specs, `DataGridElement.shared` static, `apps/docs/src/content/docs/grid/architecture.mdx` § "Multi-instance / shared store". Reverted on branch `338-rollback`.

- DECIDED (May 2026, #339): **multi-version coexistence via per-bundle auto-suffixed tags** — solves the residual problem from #338/#340 (`customElements.define('tbw-grid', …)` is realm-global, first-wins → bundle B's `<tbw-grid>` instances become bundle A's class, lose B's plugin/feature registrations, risk version-skew). Algorithm in `registerDataGrid()` ([grid.ts](libs/grid/src/lib/core/grid.ts) bottom): no existing registration → take bare `tbw-grid`; same-version existing → reuse bare (don't redefine); different-version existing → register `tbw-grid-v{version sanitised to `[a-z0-9-]+`}`. `DataGridElement.activeTag` static reflects the chosen tag. Every grid instance sets `data-tbw-grid=""` in `connectedCallback` regardless of tag — that attribute is the **stable selector contract** for CSS / `closest()` / `querySelector` ("any grid, any version"). INVARIANT: themes and adapter DOM traversal use `[data-tbw-grid]`, NEVER bare `tbw-grid` tag. Adapters render via `createElement(GridElement.activeTag, …)` (React) and `<component :is="gridTag">` (Vue) so suffixed bundles emit the right tag. Style injector scopes its `<style id="tbw-grid-styles-{activeTag}">` element to the active tag and rewrites bare `tbw-grid` selectors in the injected CSS via `(?<![-\w])tbw-grid(?![-\w])` regex (negative lookbehind avoids corrupting `[data-tbw-grid]` attribute selectors; no-op single-version). `globalThis.DataGridElement` alias is **first-wins** — gated by `if (!globalThis.DataGridElement)`; multi-version-aware code must `import { DataGridElement }` directly or `customElements.get(activeTag)`. Pragmatic legacy workaround (Planning's `GridSplit.tsx` `customElements.whenDefined('tbw-grid')` gating) is no longer needed — both bundles now coexist by registering distinct tags. Adapter `closest()` calls use the combined selector `'tbw-grid, [data-tbw-grid]'` for backwards compat with spec fixtures and any external code holding bare-tag mock elements. Files: [grid.ts](libs/grid/src/lib/core/grid.ts) (`activeTag` static, `connectedCallback` attribute set, `registerDataGrid()`), [internal/style-injector.ts](libs/grid/src/lib/core/internal/style-injector.ts) (per-tag id, `rewriteTagSelectors`), [grid-react/src/lib/data-grid.tsx](libs/grid-react/src/lib/data-grid.tsx) (`createElement`), [grid-vue/src/lib/TbwGrid.vue](libs/grid-vue/src/lib/TbwGrid.vue) (`<component :is>`), all theme CSS files in [libs/themes/](libs/themes/) (root selector `[data-tbw-grid]`). Tests: [multi-version-registration.spec.ts](libs/grid/src/lib/core/multi-version-registration.spec.ts). Docs: [apps/docs/src/content/docs/grid/guides/multi-version.mdx](apps/docs/src/content/docs/grid/guides/multi-version.mdx). Out of scope: Angular template literal `<tbw-grid>` in consumer code (single-version works; multi-version Angular needs a `*tbwGridTag` directive — future).
- DECIDED (May 2026, #324): always-on focus trap in `FocusManager` (constructed once, listeners on host survive disconnect/reconnect). Tracks the last meaningful user-focused element via capture-phase `focusin` on the grid host. Restores it on `focusout` when `relatedTarget === null` (bounce-to-body, e.g. an overlay child of `<body>` is removed while focused). `#noteFocus` SKIPS: (a) the grid host itself (artificial `tabindex=0` from keyboard handler), (b) bare `.cell` elements (cell focus is virtual, owned by `_focusRow`/`_focusCol`), (c) descendants of registered external containers (overlay closes → restore should land INSIDE the grid, not back into the about-to-be-removed overlay). Editor inputs inside `.cell.editing` ARE tracked. Restore is gated: `queueMicrotask` → only refocuses when `activeElement` is `<body>` or the grid host. Public surface: `grid.lastFocusedElement` (getter, only returns connected element) + `grid.restoreLastFocus()` (returns `boolean`). `EditingConfig.focusTrap` is **NOT deprecated** and is **not** a no-op — it remains an explicit opt-in for the stronger "lock focus inside the editing cell" behavior on intentional Tab/click outside the grid during a row edit. The two layers coexist by design: the always-on trap fixes accidental focus theft (toolpanel `<input>` losing focus on next render, ArrowUp on number-spinner stealing to a cell, body-bounce on overlay close); `EditingPlugin`'s opt-in `focusout` handler reclaims focus into the editor on intentional outside navigation. RULED OUT: calling `focusManager.destroy()` in `disconnectedCallback` (constructor only runs once → reconnect leaves trap uninstalled). `keyboard.ts` `handleGridKeyDown` early-return broadened to ignore ANY non-`.rows-body` target inside the host (toolpanel inputs/buttons, shell-header controls), and `ensureCellVisible` no longer calls `grid.focus()` when a meaningful non-cell descendant currently holds focus. Files: [focus-manager.ts](libs/grid/src/lib/core/internal/focus-manager.ts) (`#installFocusTrap`, `#noteFocus`, `#scheduleRestore`, `restoreLastFocus`), [keyboard.ts](libs/grid/src/lib/core/internal/keyboard.ts), [grid.ts](libs/grid/src/lib/core/grid.ts) (`lastFocusedElement` getter, `restoreLastFocus`), [editing/types.ts](libs/grid/src/lib/plugins/editing/types.ts) (`@remarks` describing coexistence). Tests: `__tests__/integration/focus-management.spec.ts > always-on focus trap` (6 cases) + `> focusCell preserves non-cell descendant focus` (3 cases).
