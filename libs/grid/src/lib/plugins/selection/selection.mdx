{/* AI-CONTEXT: For structured, AI-optimized documentation about this plugin,
    ...
    — section: "### SelectionPlugin" (under "## Plugin Configuration Reference") */}
...
- **Cell mode:** Click cells to select them individually. Great for spreadsheet-style interactions.
- **Row mode:** Click anywhere in a row to select the entire row.
  Use Ctrl+Click (⌘+Click on Mac) to toggle individual rows, and Shift+Click to select a contiguous range.
- **Range mode:** Click and drag to select rectangular ranges.
  Hold Ctrl (⌘ on Mac) while dragging to create multiple independent ranges. Shift+Click extends the most recent range.
...
Add `checkbox: true` to show a selection checkbox column with a "select all" header. Works exclusively in row mode. Checkboxes toggle individual rows (like Ctrl+Click), and Shift+Click between checkboxes selects the range.
When `multiSelect: false`, the header checkbox is hidden and row checkboxes replace the current selection instead of toggling.

<Canvas of={SelectionStories.CheckboxSelection} withToolbar />
...
| `Ctrl/⌘ + Click`    | Toggle row/cell without clearing (multi-select) |
| `Shift + Click`      | Extend selection from anchor to clicked row/cell|
| `Shift + Ctrl + Click` | Add contiguous range to existing selection    |
...
// Query current selection (unified across all modes)
const selection = plugin.getSelection();
// => { mode: 'row', ranges: [{ from: {row:0,col:0}, to: {row:0,col:4} }], anchor: {row:0,col:0} }

// Row mode: select specific rows by index
plugin.selectRows([0, 2, 4]);
const indices = plugin.getSelectedRowIndices(); // [0, 2, 4]

// Get actual row objects (recommended over manual index lookup)
const rows = plugin.getSelectedRows<Employee>(); // Employee[]

// Range mode: set ranges programmatically
plugin.setRanges([{ from: { row: 0, col: 0 }, to: { row: 5, col: 3 } }]);

// Check individual cells
plugin.isCellSelected(2, 1); // true/false
const cells = plugin.getSelectedCells(); // [{ row, col }, ...]

// Actions
plugin.selectAll();       // Select all (row/range modes)
plugin.clearSelection();  // Clear everything
```

> **Best practice:** Use `getSelectedRows()` to retrieve actual row data objects rather
> than resolving indices manually with `getSelectedRowIndices()`. Row indices refer to
> positions in the grid's current (sorted/filtered) row array, which may differ from
> your original data array. `getSelectedRows()` handles this mapping for you in all
> selection modes.
