...
// with a portal-aware version (`{ override: true }`), so we don't need the core
// feature's runtime registration — only its `declare module` augmentation that
// makes `filtering` a known property on `FeatureConfig`. A bare side-effect
// import (`import '@toolbox-web/grid/features/filtering'`) is stripped from the
// emitted `.d.ts`, so consumers would still see TS2353. A named type re-export
// of the `_Augmentation` anchor survives emit and drags the augmenting module
// in. See `lib/internal/feature-augmentations.ts` for the same trick.
