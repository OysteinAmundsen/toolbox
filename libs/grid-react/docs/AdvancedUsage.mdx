...
function EmployeeGrid() {
  ...
  return (
    <div>
      ...
      <DataGrid rows={employees} columns={columns} exportEnabled />
    </div>
  );
}
`}
...
function EmployeeGrid() {
  ...
  return <DataGrid rows={employees} gridConfig={config} sorting />;
}
`}
/>
...
function LargeDataGrid() {
  const config: ReactGridConfig<DataRow> = {
    columns: columns,
    virtualization: {
...
function ServerSideGrid() {
  ...
  const fetchData = useCallback(async (sort: SortState[]) => {
    setLoading(true);
    try {
  ...
  return (
    <div>
      ...
      <DataGrid
        rows={employees}
        columns={columns}
        sorting={{ mode: 'external', sortState }}
...
const config = useMemo<ReactGridConfig<Employee>>(() => ({
  columns: [/* ... */],
