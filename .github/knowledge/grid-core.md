...
- DECIDED (May 2026, #338 / #340 reverted): **do NOT introduce a `globalThis`-anchored cross-bundle shared store for grid identities (feature registry, React Context, Vue `InjectionKey`, Angular `InjectionToken`).** PR #340 shipped this on `main` and was reverted before release because the design fights the deployment shape it claims to fix. WHY: real-world micro-frontend hosts (Equinor Roma's `<AppLoader>`, Module Federation without `shared:`, single-spa root configs without shared deps) give each bundle its **own React/Vue/Angular runtime** in its own root — `createRoot(el)` per widget, mounted on a host-supplied `<div>`. Two adapter bundles on one page therefore do **not** share a reconciler tree; `useContext` / `inject` lookups never cross the boundary. Forcing Context identity sharing via `globalThis[Symbol.for('@toolbox-web/grid/shared')].reactContexts[name]` _creates_ the crash it advertises as a fix: bundle B's React reconciler receives a Context object whose internal `$$typeof` was minted by bundle A's React, and either throws "Invalid hook call" / "element type is invalid" or silently fails the `===` identity check. The `features` bucket has a separate foot-gun — bundle A's v2.13 feature factory wired to bundle B's class internals = SPA-session contamination crash. RULED OUT alternatives (do not re-propose without addressing both points): (a) versioning the shared bucket keys (still shares Context objects minted by the wrong React); (b) opt-in shared store (the failure mode is silent enough that nobody opts in until it crashes in prod). The real residual problem after `<AppLoader>`-style isolation is `customElements.define('tbw-grid', …)` being **realm-global, first-wins** — bundle B's `<tbw-grid>` instances are bundle A's class, consult A's module-private feature registry, silently drop B's registered features (TBW031), and risk version-skew between A's class and B's React wrapper. The correct fix is per-bundle tag suffixing (issue #339, "always-suffix per bundle" variant — each copy of the grid gets `tbw-grid-{bundleId}` via a build-time-injected nonce; adapters render `createElement(activeTag, …)`). The current pragmatic workaround in Planning's `GridSplit.tsx` is `customElements.whenDefined('tbw-grid')` gating to deterministically pick the registration winner — see `c:/dev/planning/apps/planning/src/planning/home/object-graph/grid-split/GridSplit.tsx`. Files removed in revert: `libs/grid/src/lib/core/internal/shared-store.ts`, `*.spec.ts` companions, adapter `shared-*` specs, `DataGridElement.shared` static, `apps/docs/src/content/docs/grid/architecture.mdx` § "Multi-instance / shared store". Reverted in commit on branch `338-rollback`.
