...
Some tools (including Copilot fetch in certain environments) may time out or truncate very large text files such as `llms-full-angular.txt`. In that case, use a local retrieval workflow instead of pasting the whole corpus into the model:

1. Download once to a local file.

```bash
curl -L https://toolboxjs.com/llms-full-angular.txt -o llms-full-angular.txt
```

2. Search locally for the exact topic, symbols, or plugin names.

```bash
rg -n "renderer|headerRenderer|gridConfig\.features|SelectionPlugin|TbwRenderer|GridColumn" llms-full-angular.txt
```

3. Extract only the relevant region(s) and provide those chunks to the assistant.

```bash
sed -n '1200,1320p' llms-full-angular.txt
```

This gives predictable, low-latency retrieval and avoids context-window overload. As a rule: **search first, then send only the matching slices**.
