...
The grid provides built-in loading indicators at three levels for visual feedback during async operations.

| Level | Use Case | Visual |
|-------|----------|--------|
| **Grid** | Initial data fetch, refresh | Large centered spinner with backdrop |
| **Row** | Saving row, row-level async operations | Small spinner on row |
| **Cell** | Cell validation, cell-level async operations | Tiny spinner on cell |

All loading states are **user-instructed** - you control when to show/hide them.

### Grid-Level Loading

Show a full-grid loading overlay during initial data fetch or refresh operations:

<Canvas of={LoadingStories.GridLoading} withToolbar />

**Realistic data fetch example:**

<Canvas of={LoadingStories.FetchDataDemo} withToolbar />

### Row-Level Loading

Show loading indicators on specific rows during save operations:

<Canvas of={LoadingStories.RowLoading} withToolbar />

### Cell-Level Loading

Show loading indicators on individual cells during validation or async operations:

<Canvas of={LoadingStories.CellLoading} withToolbar />

### Custom Loading Renderer

Replace the default spinner with your own component using `loadingRenderer`:

<Canvas of={LoadingStories.CustomLoadingRenderer} withToolbar />
