// @internal Types-only barrel: pulls core feature modules onto the type
// graph (vite-plugin-dts / typedoc) so FeatureConfig augmentations merge
// before emit. See .github/knowledge/adapters-angular.md ("FeatureName
// derivation"); React/Vue use the same trick — without these re-exports,
// `keyof FeatureConfig` collapses (typedoc only sees features that are
// transitively imported from its entry points), breaking `FeatureName`.
//
// Each line re-exports the core feature's `_Augmentation` anchor type under
// a unique local name. This is functionally equivalent to
