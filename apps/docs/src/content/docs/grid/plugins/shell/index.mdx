...
:::note[Opt-in]
The shell is opt-in and tree-shakeable. Enable it with
`import '@toolbox-web/grid/features/shell'` + `features: { shell: true }` (or by registering
`ShellPlugin` directly). Nothing shell-related is bundled unless you import it.
:::
...
| `toolPanel.defaultOpen` | `string` | — | Accordion section to auto-expand on first open. Also opens the sidebar on load — see [#259](https://github.com/OysteinAmundsen/toolbox/issues/259) for the planned split. Prefer `initialState` / `locked` to control the open state and `defaultOpen` purely for section selection. |
| `toolPanel.initialState` | `'open' \| 'closed'` | `'closed'` | Whether the sidebar starts open on grid load. Takes precedence over `defaultOpen`'s open-on-load behavior when set. |
...
## Migrating from the Core Shell (pre-v3)
...
package root. **Those surfaces were removed in v3.0.0.** The `shell` configuration object itself
is still augmented onto `gridConfig` by the plugin. Migrate as follows:
...
- **Adapters no longer auto-register** the `ShellPlugin`. React/Vue/Angular users must add the
  adapter side-effect import `import '@toolbox-web/grid-{react,vue,angular}/features/shell'`.
- Shell usage without the feature/plugin enabled throws a clear error with a migration link.
- See the [v3 migration guide](/grid/guides/migration-v3/) for the full change set.

## Accessibility

The tool panel is resized by dragging the splitter between the panel and the grid. WCAG 2.2 [SC 2.5.7 Dragging Movements](https://www.w3.org/WAI/WCAG22/Understanding/dragging-movements.html) requires a single-pointer alternative for every such gesture, so the splitter is a button as well as a drag target.

- **Tap the splitter** — press and release without moving and the size popover opens against it, with − / a width field / + / *Reset*. Dragging behaves exactly as before; the two are told apart by whether the pointer moved.
- **The same popover used for columns** — it is the shared control from [column resizing](/grid/guides/accessibility/#pointer-alternatives-to-dragging), so the gesture, keyboard behaviour, and appearance are identical wherever something is resizable. − and + step by 32px, the field takes an exact pixel value, and *Reset* drops the explicit width. `Escape`, or a press anywhere outside, dismisses it.
- **Same limits as the drag** — every path through the popover clamps to the same 200px minimum and the same available maximum, so no pointer alternative can produce a size the drag could not.
- **No reserved chrome** — nothing extra is drawn. The alternative reuses the affordance you would have dragged, and the splitter carries `role="button"` with an accessible name instead of `aria-hidden`.
- **A 24px target on every pointer** — [SC 2.5.8 Target Size (Minimum)](https://www.w3.org/WAI/WCAG22/Understanding/target-size-minimum.html) applies to a mouse as well as a finger, so the splitter's invisible hit area is 24px wide while the lit bar stays 6px. It straddles the docked edge, half over the grid and half over the panel, so it never covers the panel's own controls. Raising `--tbw-touch-target-min` for the rest of the grid? Raise `--tbw-tool-panel-resize-overhang` to half that value as well — see [Hit-target sizing](/grid/guides/touch-input/#hit-target-sizing).

See the [accessibility guide](/grid/guides/accessibility/#pointer-alternatives-to-dragging) for the full inventory of drag alternatives across the grid.
