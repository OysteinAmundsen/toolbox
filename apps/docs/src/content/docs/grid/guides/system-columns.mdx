...
The column **is** rendered in the grid (header + cells, with your `renderer` /
`viewRenderer` / `headerRenderer`). It is excluded from everything else:
...
- **Drag handle** — a visual handle for row reorder (the built-in `RowDragDropPlugin` already does this)
...
| `RowDragDropPlugin` | Drag handle column                                      |
