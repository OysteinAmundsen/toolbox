...
- CONSEQUENCE: the duck-typed seam (`getPluginByName('shell') as { disposeShellState?(): void }`) makes plugin members invisible to static analysis. `MasterDetailPlugin.refreshDetailRenderer`, `RowDragDropPlugin.emitTransfer`, `ShellPlugin.disposeShellState` are LIVE despite being reported "unused". **Grep the member name as a string before deleting anything fallow flags.**
- DECIDED (audit 2026-08-07): Nx tag `depConstraints` CANNOT enforce boundaries here. WHY: tsconfig `paths` for `@toolbox-web/grid[/subpath]` point at `dist/`, so Nx can't attribute those imports to the `grid` project and flags every legitimate adapter→core import — the same resolver limitation that forced `enforceBuildableLibDependency: false`. Projects still carry `layer:core|adapter|styles` + `type:lib|app|e2e` tags for `nx affected` filtering; the adapter↔adapter boundary is enforced by a graph-independent `no-restricted-imports` rule. RULED OUT: re-attempting tag constraints without first changing the dist-pointing paths.
...
- DECIDED (#324, always-on focus trap): `FocusManager` constructed once; host listeners survive disconnect/reconnect. Tracks last meaningful user-focused element via capture-phase `focusin`; restores on `focusout` when `relatedTarget === null` (body bounce). `#noteFocus` SKIPS: the grid host itself, a bare `.cell` (cell focus is virtual via `_focusRow`/`_focusCol`), and descendants of registered external containers. Editor inputs inside `.cell.editing` ARE tracked. Restore is `queueMicrotask`-gated to when `activeElement` is `<body>` or the host. Public: `grid.lastFocusedElement`, `grid.restoreLastFocus(): boolean`.
  ...
  - RULED OUT: `focusManager.destroy()` (constructor runs once → reconnect leaves the trap uninstalled). The method + its `#trapCleanup`/`AbortController` were DELETED (Aug 2026 audit) — the trap's two listeners are on the grid host, so they die with the element. Do not re-add.
