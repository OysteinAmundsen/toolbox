 ...
 * config (e.g. Vue components passed where the core plugin expects a
 * renderer function).
 *
 ...
 *
 * The `<TbwGrid>` shell does NOT currently invoke this registry — Vue
 * feature configs flow directly through `createPluginsFromFeatures` and
 * Vue component values are already framework-shaped. Shipping the surface
 * now means third-party features can target a stable API across all three
 * adapters today, and the shell can opt-in to invoking it later without an
 * API break (matches the Phase 3 `child-feature-detector` ship-without-caller
 * approach).
 *
...
import type { GridAdapter } from './vue-grid-adapter';
