...
function EmployeeGrid({ data }) {
  ...
  return (
    <div>
      ...
      <DataGrid rows={data} gridConfig={gridConfig} style={{ height: '400px' }} />
    </div>
  );
}
```
...
You can instead pass `columns={[ … ]}` and `print={{ … }}` as individual props on `<DataGrid>`. That form is idiomatic React, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
  <TabItem label="Vue">
```html
...
You can instead bind `:print="{ … }"` and declare columns with `<TbwGridColumn>` children. That form is idiomatic Vue, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
...
export class EmployeeGridComponent {
  ...
  gridConfig: GridConfig = {
    ...
    features: { print: { title: 'Employee Report', includeTitle: true, includeTimestamp: true } },
  };
...
You can instead import `GridPrintDirective` and bind `[print]` directly on `<tbw-grid>`. That form gives a compile-time check that the feature is imported, but `gridConfig.features` above is the recommended, framework-consistent approach.
:::
  </TabItem>
</Tabs>
...
Columns marked with [`utility: true`](/grid/core/#system-columns) — including the
...
Every option passed to `print()` overrides the plugin-level configuration for that one call.
See [`PrintParams`](./interfaces/printparams/) for the full option list, and
[`PrintConfig`](./interfaces/printconfig/) for the defaults those overrides apply on top of.
