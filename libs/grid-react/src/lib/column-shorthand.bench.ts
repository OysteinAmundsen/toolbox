/**
 * Column-shorthand Benchmarks (React adapter)
 *
 * `normalizeColumns` runs on every render of `<DataGrid>` because columns
 * arrays are typically inline-defined in JSX. A regression here scales with
 * column count × component-tree updates, so it's worth tracking.
 */
