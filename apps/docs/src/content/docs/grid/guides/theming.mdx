...
For **any other icon**, switching to mask mode requires three steps:

1. Set the content variable to `''` (empty string) so the pseudo-element is generated but shows no text
2. Set the `-mask` variable to an SVG `url()`
3. Add your own CSS rule for `background`, `mask-size`, `mask-repeat`, and `mask-position` — these are only built-in for the filter icons

```css
@layer tbw-theme {
  tbw-grid {
    /* Step 1 & 2: set content to '' and provide the mask URL */
    --tbw-icon-sort-asc: '';
    --tbw-icon-sort-asc-mask: url("data:image/svg+xml,...");
  }

  /* Step 3: add mask rendering rules for this icon */
  tbw-grid [data-icon='sort-asc']:empty::before {
    background: currentColor;
    -webkit-mask-image: var(--tbw-icon-sort-asc-mask);
    mask-image: var(--tbw-icon-sort-asc-mask);
...
:::tip
The grid intentionally ships mask CSS only for filter icons to keep the base bundle small. The pattern above gives theme authors full control to opt in per icon.
:::
