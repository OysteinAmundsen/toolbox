...
> **Note:** The `renderer` property is a **core feature** that works without any plugins. Only `editor` and `editorParams` require the EditingPlugin.

See the [Core Grid docs](?path=/docs/grid-core--docs#type-level-defaults) for renderer examples, or the [Editing Plugin docs](?path=/docs/grid-plugins-editing--docs#type-level-defaults) for editor examples.
