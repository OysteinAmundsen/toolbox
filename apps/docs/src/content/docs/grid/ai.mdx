...
All of these formats follow the [llms.txt standard](https://llmstxt.org/) and are served from the site root.

<CardGrid>
  <LinkCard
    title="llms.txt"
    description="A concise, curated index: a one-paragraph overview, the agent steering rules, a task-to-page routing table, and links to every documentation page (each pointing at its Markdown companion). Small enough to fit a modest context window."
    href="/llms.txt"
  />
  <LinkCard
    title="llms-api.txt"
    description="The exhaustive generated TypeDoc index: every class, interface, type, function, and variable linked to its Markdown companion. Split out of llms.txt so the index stays cheap to read; fetch this when you need an exact type signature."
    href="/llms-api.txt"
  />
  <LinkCard
...
`llms-full.txt` is the **full corpus**: every prose guide, plugin, and adapter page concatenated with demo code inlined, for one-shot ingestion. It opens with a `## Contents` map — every page with a one-line summary and its single-page `.md` URL — so an agent that greps the file can tell where a hit landed and fetch just that page instead. The per-symbol API reference is linked, not inlined. A per-framework variant drops the other stacks' examples to trim size.
...
| **Need an exact type signature**                              | **`llms-api.txt`**                    | Every generated TypeDoc symbol linked to its `.md` companion, grouped by area.                    |

**Recommended default: `llms.txt`.** It is the curated map (overview, the agent steering rules, and a link to every page's `.md` companion). A capable agent reads it, then fetches just the handful of pages a task requires — e.g. the Editing and Selection plugin pages for an editable grid — instead of ingesting all {GRID_STATS.pluginCount} plugins. Point at **`llms-full.txt` only** when your tool can't fetch on demand and you want everything in a single load. If your tool can't fetch but you only target one framework, prefer the scoped **`llms-full-{react,vue,angular,vanilla}.txt`** variant — it is the same corpus with the irrelevant framework variants removed.
...
- **Honest curation** — to keep the corpus focused on _building_ with the grid, a few non-implementation pages (this one, the changelogs, the competitive comparison, the v1→v2 migration guide, the multi-version coexistence guide, and the full-app demos showcase) are kept out of `llms-full.txt`, and within the pages that remain the illustrative `Demos`/`See Also` sections are dropped (their large inlined demo dumps and link lists carry no API truth). **Plugin pages are condensed to a stub** — intro, `Installation`, and `Basic Usage` — so an agent learns which plugin solves a need and how to enable it inline; each plugin's deep API (configuration options, events, methods) is linked as a `.md` companion to fetch on demand rather than inlined, since plugins otherwise dominate corpus size. Every API-bearing section of the core and guide pages — installation imports, configuration options, programmatic APIs, events, keyboard shortcuts — is retained. All omitted pages and full plugin docs stay linked in `llms.txt`, and each page's standalone `.md` companion keeps its full content and demos.
