...
### `render` — render cycle completed

Fires once at the end of every render cycle (the single RAF flush in the render
scheduler), **after** all plugin `afterRender` hooks have run and **after**
`grid.ready()` has resolved. Use this when you need to act on the rendered DOM
immediately after a programmatic mutation — without resorting to `setTimeout`
or double-`requestAnimationFrame` hacks.

:::tip[`ready()` vs `render` event]
`grid.ready()` resolves **once**, after the first render. The `render` event
fires on **every** flush. When you only care about a specific mutation (e.g.
the row you just added), attach with `{ once: true }`.
:::

The detail payload includes the highest render phase that ran (`phase`),
whether this was the first render (`initial`), the post-`processRows` row
count (`rowCount`), and the current virtual window (`visibleRange`, or `null`
when virtualization is disabled).

#### Focus the first input after `addRow` in full-grid edit mode

The original motivation: with `editing: { mode: 'grid' }` every row is
permanently in edit mode. After inserting a new row you want the first cell's
input focused — but the row doesn't exist in the DOM until the next render.
...
The event fires on virtualization-only re-renders too. If you only care about
row/column model changes, gate on `phase`:
...
See the [Theming](/grid/guides/theming/) page for the complete list of CSS custom properties.
...
grid.gridConfig = {
  shell: {
    header: {
      ...
      toolPanelToggle: true,        // Render built-in panel toggle button (default: true)
    },
    toolPanel: {
      ...
      initialState: 'open',         // 'open' | 'closed' — preferred way to control sidebar open state
      locked: false,                // Lock the sidebar open and hide the toggle button
...
Set `header.toolPanelToggle` to `false` to suppress the built-in `<button class="tbw-toolbar-btn" data-panel-toggle>` and the adjacent `.tbw-toolbar-separator` — useful when supplying your own design-system toggle button. Wire the custom button to `grid.toggleToolPanel()` or `grid.toggleToolPanelSection(id)`. See [Bring Your Own Tool-Panel Toggle](./core#bring-your-own-tool-panel-toggle) for a worked example.
