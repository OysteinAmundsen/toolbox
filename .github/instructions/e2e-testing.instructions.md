...
| Sticky rows           | Clones satisfy `dataRows()`/`rowCount()` — filter with `:not(.tbw-sticky-row)`. Switching `mode` rebuilds the dataset and resets scroll: wait for the clone count to drop to `0` **before** scrolling again, then scroll in bounded `expect.poll` passes rather than one fixed delta (a slow runner coalesces wheel events; `maxStacked` caps the result so over-scrolling is safe) |
