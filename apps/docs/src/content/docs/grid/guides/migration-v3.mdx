...
both adapters. The prop was a no-op — `<tbw-grid>` is a custom element that
hydrates normally on the client regardless of the flag. **Delete any `ssr` prop
and `SSRProps` import;** no replacement is needed.
...
For per-framework client-only boundary recipes (Next.js, Remix, Nuxt, Astro,
Angular SSR), see
[Platform Support → Server-Side Rendering](/grid/guides/platform/#server-side-rendering).
