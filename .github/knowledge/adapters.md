...
- DECIDED (#330, May 2026): `FrameworkAdapter.beginBatch?()` / `endBatch?()` are the bulk-teardown contract. Grid core wraps multi-`releaseCell` loops where every affected cell is detached before the batch ends — `_clearRowPool`, `renderVisibleRows` pool-shrink, `renderInlineRow` pre-wipe (all in `core/grid.ts` + `core/internal/rows.ts`). Adapters that synchronously commit framework teardown per cell defer commits between `beginBatch` / `endBatch`; adapters that already batch (Vue, Angular) implement them as no-ops for parity. Listed in `CORE_CONSUMED_ADAPTER_METHODS`. Calls nest — outermost `endBatch` is the only one that flushes.
...
- DECIDED (May 2026): `createNodeBridge` MUST `instanceof Node` check the value returned by the user fn and pass DOM nodes through unwrapped. WHY: vanilla pinned-row helpers (`rowCountPanel()`, `filteredCountPanel()` from `@toolbox-web/grid/plugins/pinned-rows`) return `(ctx) => HTMLElement`. Without the check React tries to render the element as a child and `PortalBoundary` traps `Objects are not valid as a React child (found: [object HTMLSpanElement])`. Mirror in Vue's `teleport-bridge.ts`.
...
- DECIDED (May 2026, employee-management refactor): `detectChildComponentFeatures` in `data-grid.tsx` auto-derives `responsive: true` / `masterDetail: { showExpandColumn, animation }` from `<GridResponsiveCard>` / `<GridDetailPanel>` children. `mergedFeatureProps` MUST strip child-detected entries that are already keyed in `gridConfig.features`. WHY: child-derived configs are bare-bones (`responsive: true` has no `breakpoint`); the resulting plugin lands in `processedGridConfig.plugins`; core's manual-wins dedup (`grid.ts` `#initializePlugins`) then drops the user's full `gridConfig.features.responsive` instance, leaving the no-breakpoint version → TBW110. Same risk for masterDetail.
...
- DECIDED (#256): `createNodeBridge<TCtx>(vueFn)` in `teleport-bridge.ts` mirrors React's helper exactly. Same `?? createElement('div')` coercion; `features/filtering.ts` left inline for the same reason. Same Node-passthrough rule as React (see react-adapter — vanilla pinned-row helpers return real DOM, mounting through Vue's renderer would error).
- DECIDED (May 2026): `TbwGrid.vue` MUST set `defineOptions({ inheritAttrs: false })` and apply `v-bind="$attrs"` on the inner `<tbw-grid>`. WHY: component renders fragment root (`<TeleportManager/>` + `<tbw-grid>`); without this, any `class=` / unknown attribute on `<TbwGrid>` triggers "Extraneous non-props attributes" warnings. Forwarding to the inner element preserves the natural fall-through users expect.
- DECIDED (May 2026): `TbwGridToolPanel.vue` accepts BOTH `title` (canonical, mapped to the `title` attribute the core grid reads) and `label` (deprecated alias kept for old templates). WHY: previously only `label` was accepted and forwarded as `:label=`, but `core/internal/shell.ts > parseLightDomToolPanels` reads `title` only. Result: every Vue tool panel silently failed with TBW070 and was dropped. React/Angular adapters always used `title` — the bug was Vue-only and unreported because no shipped app had wired up `<TbwGridToolPanel>` until the unified employee-management demo.
- DECIDED (May 2026): `parseLightDomToolPanels` MUST NOT tear down adapter-rendered panel content on idempotent re-parses. INVARIANT: on a re-parse of an existing panel that ALREADY had an adapter renderer bound, only refresh the stored `render` closure + attributes; do NOT run the existing `panelCleanups` cleanup unless (a) this is the first adapter attach (vanilla fallback → adapter) or (b) a header attribute (order/icon/tooltip) changed. WHY: every `grid.gridConfig = …` setter routes through `#applyGridConfigUpdate` → `parseLightDomToolPanels`. Before this fix the cleanup ran unconditionally, which unmounted the user's React/Vue component inside `<GridToolPanel>` on every config sync — destroying local state and resetting `scrollTop` of any scrollable custom panel. Tracking set: `ShellState.adapterBoundToolPanelIds`. Files: `core/internal/shell.ts`, regression test in `shell.spec.ts` ("does not tear down adapter-rendered panel content on idempotent re-parse").
...
- DECIDED (employee-management refactor): `gridConfig.features` and per-feature directives compose. When a directive's selector matches **only** via an event binding (e.g. `<tbw-grid (filterChange)="..."/>` without `[filtering]`), `registerFeatureClaim(grid, 'filtering', () => this.filtering())` returns `undefined` → `Grid.createFeaturePlugins` skips that name → core's `createPluginsFromFeatures(gridConfig.features)` creates the plugin → directive's `addEventListener('filter-change', …)` still fires. Lets demos move feature config out of templates while keeping framework event hooks. Demo: `demos/angular/src/demos/employee-management/`.
...
- DECIDED (May 2026, employee-management refactor): Angular `registerTemplateBridge` callbacks for `master-detail` and `responsive` MUST look up the existing plugin via `(grid as any).getPluginByName?.(name)`, NOT by scanning `gridConfig.plugins`. WHY: plugins instantiated from `gridConfig.features.<name>` never appear in `gridConfig.plugins` — they're produced by core's feature resolver and live only in `PluginManager`. Using `gridConfig.plugins.find(...)` made the bridge spuriously emit "is not configured" warnings for features the user had configured correctly via `gridConfig.features`.
...
- DECIDED (#330, May 2026): `PortalManager` exposes `beginBatch()` / `endBatch()` on its handle; `portal-bridge.ts` re-exports `beginPortalBatch` / `endPortalBatch`; `react-grid-adapter.GridAdapter.beginBatch/endBatch` delegate to those. While a batch is open, `removePortal(key, sync=true)` deletes the entry but SKIPS `flushSync` — the caller has promised to detach the affected container before `endBatch()` runs, so the next deferred render's `isConnected` filter prunes the entry without committing an unmount. Eliminates the `flushSync was called from inside a lifecycle method` warning storm during grouping changes (N visible rows × M React-rendered columns warnings → 0). INVARIANT: bulk-teardown call sites in grid core MUST detach every affected container (via `bodyEl.innerHTML = ''`, `el.remove()`, or `rowEl.innerHTML = ''`) AFTER the per-cell release loop and BEFORE `endBatch()`. Otherwise the deferred render commits an unmount against a container whose children were already wiped, reproducing the original `removeChild` `NotFoundError` from #250. Test: `portal-manager.spec.tsx > beginBatch / endBatch`.
- DECIDED (#332, May 2026): `PortalManager` guards every deferred path with an `unmountedRef` set in a `useEffect` cleanup. WHY: when the host React tree containing `<DataGrid>` unmounts (route change, view toggle, conditional render), pending `queueMicrotask` (`scheduleFlush`) and rAF (`schedulePrune`) callbacks would otherwise fire AFTER unmount and re-enter `createPortal(...)` against torn-down providers — every cell whose subtree calls a context hook (`useQuery`, `useAppModule`, `useTranslation`, `useContext(...)`) throws an uncaught exception, one per visible row × per React-renderer column. Cleanup sets `unmountedRef = true`, cancels the rAF prune, and clears `portalsRef`; all imperative methods (`renderPortal` / `removePortal` / `clear`) become no-ops post-unmount. INVARIANT: portal subtrees MUST be dropped from React's tree alongside the manager itself — no orphan commits. NOT MIRRORED in Vue/Angular: Vue's `TeleportManager` is structurally safe (Vue has no `flushSync`; writing to a `ref` of an unmounted component is a no-op; `onBeforeUnmount` already cancels its rAF). Angular has no portal-manager — embedded views are tied directly to `ViewContainerRef` lifecycle. Test: `portal-manager.spec.tsx > host-tree unmount cleanup`.
