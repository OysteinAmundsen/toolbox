...
- **Export.** The [Export plugin](/grid/plugins/export/) writes CSV and Excel XML. If your data
  can contain values starting with `=`, `+`, `-`, or `@`, sanitize them to avoid
  [CSV injection](https://owasp.org/www-community/attacks/CSV_Injection) when the file is
...
| Filter panel labels | Filtering plugin config, or a custom `filterPanelRenderer` |
