...
- DECIDED (#304, Jul 2026): **touch selection mode** — long-press a row → mode on + that row selected; plain taps then toggle (treated as Ctrl); a second long-press range-extends from the anchor; `Done` / `Escape` exits. State: `touchSelectionActive` getter + `exitTouchSelection()`. Config `SelectionConfig.touchMode: 'sticky' | 'transient'` (default `'transient'` = exiting clears). `@since 3.5.0`.
  ...
  - Long-press detection is FREE: `event-delegation.ts` only dispatches the `mousedown` hook for a coarse pointer *after* 400 ms (#303), so reaching `onCellMouseDown` with a coarse event already means "held". SelectionPlugin adds no timer of its own.
  - Header presses (`rowIndex < 0`) are ignored — reserved for the column header menu (#270, not yet built).
  - UI lives in `plugins/selection/touch-selection.ts`: `SelectionToolbar` (role=toolbar, `data-action` buttons) and `RangeCornerHandles` (two `startPointerDrag` dots on the active range; **additive** — mouse users get them too; hidden when the anchor cell is virtualized away). "More…" proxies to `ContextMenuPlugin.showMenu()` via `getPluginByName('contextMenu')`, hidden when absent.
