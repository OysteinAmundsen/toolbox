...
- DECIDED (gh #356 phase 4): `registerFeaturePropKey(name)` / `getFeaturePropKeys()` registry replaces the hand-maintained `FEATURE_KEYS` array in `data-grid.tsx` and `FEATURE_PROPS` array in `TbwGrid.vue`. Pre-populated at module load with every built-in name (24 in React, 23 in Vue — pre-existing parity gap kept as-is; not Phase 4's job to fix). WHY: third-party features that augment `FeatureConfig` no longer need to fork the shell to have their prop iterated for plugin creation. `Map<FeatureName, true>` storage so re-registering is a no-op replace (HMR-friendly), `getFeaturePropKeys()` returns a frozen array snapshot so callers cannot mutate by reference. Angular skipped — uses per-feature `@Input()` directives, no central iteration to extract. Tests: `feature-prop-keys.spec.ts` in React + Vue.
