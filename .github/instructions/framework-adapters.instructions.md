...
- **Cross-adapter registry parity (gh #356)** — Each adapter ships ONLY the registries its shell actually invokes; "ship-without-caller for API parity" is dead code (gh #356 phase 7 cleanup). Per-adapter map: React = `registerEditorMountHook`, `registerPostMountRefresh`, `registerChildFeatureDetector`, `registerFeaturePropKey`. Vue = `registerEditorMountHook`, `registerPostMountRefresh`, `registerFeaturePropKey`. Angular = `registerEditorMountHook`, `registerFeatureConfigPreprocessor` + `getFeatureConfigPreprocessor`. Asymmetry is framework-shaped: Angular's `ngAfterContentInit` covers post-mount; Angular's per-feature directives replace child-scan + prop-spread; Angular's `TemplateRef`/`Component` shapes need bridging that JSX/VNode do not. When adding a new registry, place it on the adapter whose shell will invoke it — and only on that adapter. Parity is asserted at runtime by `*-grid-adapter.registry-parity.spec.ts` (positive + negative assertions per adapter). Within an adapter, never remove or rename an exposed registry without a deprecation cycle.
...
- **Adapter type names must mirror the core type name — no framework prefix** — When an adapter widens a core config type to accept framework-native renderers (JSX/VNode/`Type<>`), the public re-export MUST use the SAME canonical name as the core type (e.g. `MasterDetailConfig`, `ResponsivePluginConfig`, `FilterConfig`, `PinnedRowsConfig`, `GroupingColumnsConfig`). The colliding core import is renamed to `Core*` (`MasterDetailConfig as CoreMasterDetailConfig`). Rationale: consumers import a single canonical name regardless of framework — `import { MasterDetailConfig } from '@toolbox-web/grid-react'` and `import { MasterDetailConfig } from '@toolbox-web/grid'` describe the same surface; the former just accepts more renderer shapes. Mirrors the established precedent for `GridConfig` and `DataGridElement`. NEVER ship `ReactFooConfig` / `VueFooConfig` / `AngularFooConfig` as the canonical name. If a prefixed name was shipped historically, keep it as a `@deprecated` type-alias to the unprefixed canonical type for one or two minor cycles, then remove.
