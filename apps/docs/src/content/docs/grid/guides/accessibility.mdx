...
- **Focus is never hidden** — plugins that paint over the rows viewport (sticky rows, pinned columns) report the space they obscure, so keyboard navigation scrolls the focused cell clear of them rather than underneath.
