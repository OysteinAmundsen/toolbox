...
See [Hit-target sizing](/grid/guides/touch-input/#hit-target-sizing) for why the splitter needs its
own token.
...
*every* Level A and AA criterion in WCAG 2.2, including the ones that are not applicable to a
component and the ones where your own content decides the outcome.
...
| 1.4.10 Reflow | AA | ✅ | Nothing around the table needs a second scrollbar at 320 px; the table itself uses the data-table exception — see [Reflow and text spacing](#reflow-and-text-spacing) |
| 1.4.11 Non-text Contrast | AA | ✅ | Focus indicators visible at 3:1 |
| 1.4.12 Text Spacing | AA | ✅ | Rows grow to fit user spacing, and text an ellipsis hides stays readable — see [Reflow and text spacing](#reflow-and-text-spacing) |
| 1.4.13 Content on Hover or Focus | AA | ✅ | [Tooltips](/grid/plugins/tooltip/#accessibility) are dismissible (<kbd>Escape</kbd>), hoverable, persistent, and shown on keyboard focus |
...
| 2.4.11 Focus Not Obscured (Min) | AA (2.2) | ✅ | Keyboard navigation scrolls the focused row clear of overlays such as [sticky rows](/grid/plugins/sticky-rows/), and the focused cell clear of [pinned columns](/grid/plugins/pinned-columns/) |
| 2.5.7 Dragging Movements | AA (2.2) | ✅ | Every drag has a single-pointer alternative — see [Pointer Alternatives to Dragging](#pointer-alternatives-to-dragging) |
| 2.5.8 Target Size (Minimum) | AA (2.2) | ✅ | Every control answers a pointer across at least 24 × 24 px, on a mouse as well as a finger — see [Target size](#target-size) |
