...
:::caution[Deprecated]
`RowReorderPlugin` is now an alias for the new
[`RowDragDropPlugin`](/grid/plugins/row-drag-drop/), which is a strict
superset (intra-grid reorder + opt-in cross-grid drag). Existing imports
keep working until V3 — see the
