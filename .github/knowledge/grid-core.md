...
- DECIDED (Apr 2026): grid renders into **light DOM**, not shadow DOM. `#renderRoot` returns `this`. Plugins and internal code MUST query the host directly (`gridEl.querySelector('.rows-body')`) — `gridEl.shadowRoot` is always null. The legacy term "shadow DOM tree" in this file refers to the *render tree structure*, not an actual ShadowRoot. Rationale: simpler styling story (user CSS + theme variables work without `::part()` everywhere), easier integration with frameworks that don't play well with shadow boundaries, no slotting complexity. Trade-off: no encapsulation — plugin styles use `tbw-grid` selector prefix to scope.
