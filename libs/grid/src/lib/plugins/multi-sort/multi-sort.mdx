...
exactly where you placed it (e.g., after a user clicks "Add Row"), call
`suspendProcessing()` before the assignment:

```ts
data.splice(3, 0, newRow);
grid.suspendProcessing(); // skip sort for this one update
grid.rows = data;
```

The flag auto-resets — the next `rows` assignment sorts normally. See the
...
> WebSocket updates). In those cases you want re-sorting to keep the grid consistent.
