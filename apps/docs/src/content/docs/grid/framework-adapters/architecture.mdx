...
For an introduction and links to the official adapters, see [Framework Adapters](/grid/framework-adapters/). For grid-core internals see [Architecture](/grid/architecture/). For typed signatures see the [`FrameworkAdapter` API reference](/grid/api/framework-adapters/interfaces/frameworkadapter/).
