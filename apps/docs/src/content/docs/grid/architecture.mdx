...
| -------- | -------- | ---- |
| `customElements.define('tbw-grid', …)` | First-wins | Both apps' `<tbw-grid>` elements run the **older** version's class even if the newer was bundled. Methods added in v2.15 are `undefined` on instances created by v2.14's code path. |
