...
- **[llms.txt](/llms.txt)** — Concise overview with links to all documentation
- **[llms-full.txt](/llms-full.txt)** — Comprehensive implementation guide with code examples, CSS variables, and plugin patterns

These are generated from the documentation on every build, so they always reflect the current docs.
