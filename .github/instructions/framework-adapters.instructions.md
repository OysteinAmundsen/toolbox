...
- **Cross-adapter registry parity (gh #356)** — When adding a registry function to one adapter, mirror it to the other two in the same PR. Export name and signature MUST match exactly. The canonical registries (re-exported from each adapter's primary `index.ts`) are: `registerEditorMountHook`, `registerPostMountRefresh`, `registerFeatureConfigPreprocessor` + `getFeatureConfigPreprocessor` (all three adapters); `registerChildFeatureDetector` and `registerFeaturePropKey` (React + Vue only — Angular uses attribute-selector directives and per-feature `@Input()`). Parity is asserted at runtime by `*-grid-adapter.registry-parity.spec.ts` in each adapter. Additive contract only — never remove or rename a registry export.
