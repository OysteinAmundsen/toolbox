...
## shell-plugin (extraction #370 — Phase 0 seam contract)

> Shell = header bar + toolbar content + tool panels. Being moved out of core into a built-in, auto-registered `ShellPlugin` (`libs/grid/src/lib/plugins/shell/`); core shell API deprecated now, dropped at v3. Decisions locked in Phase 0 before any code moves.

- DECIDED (Step 0.1, registry-delegation contract): the 16 `@deprecated` grid-element shell methods (`openToolPanel`/`closeToolPanel`/`toggleToolPanel`/`toggleToolPanelSection`/`getToolPanels`/`registerToolPanel`/`unregisterToolPanel`/`getHeaderContents`/`registerHeaderContent`/`unregisterHeaderContent`/`getToolbarContents`/`registerToolbarContent`/`unregisterToolbarContent`/`refreshShellHeader` + getters `isToolPanelOpen`/`expandedToolPanelSections`) forward to `this.getPluginByName('shell')?.<same-name>(...)`. VERIFIED: `getPluginByName<K>` exists ([grid.ts](libs/grid/src/lib/core/grid.ts#L913)), returns `PluginNameMap[K]` — ShellPlugin adds `shell: ShellPlugin` to `PluginNameMap`. The plugin re-exposes ShellController's API surface as same-named public methods so the delegates are 1-line forwards.
- DECIDED (Step 0.2, DOM-ownership transfer — RISKIEST change): shell DOM (`.tbw-shell-header` + `.tbw-shell-body`, both built by `dom-builder.ts` `buildShellHeader`/`buildShellBody`, woven in via `buildGridDOM`'s `hasShell` branch → `.tbw-grid-root.has-shell`) moves INTO the plugin. Post-#370: core's `buildGridDOM`/`buildGridDOMIntoElement` builds ONLY the un-shelled content (`.tbw-grid-root > .tbw-grid-content`); ShellPlugin restructures/wraps `.tbw-shell-body` around the content container in `attach()` (before first paint) and tears it down in `detach()`. WHY Phase 1 splits 1a (relocate, core still drives) / 1b (invert ownership): inverting DOM + state ownership is where regressions hide.
- DECIDED (Step 0.3, auto-register site + dedup): in `#initializePlugins` ([grid.ts](libs/grid/src/lib/core/grid.ts#L1007)), AFTER the existing feature/explicit `allPlugins` merge (L1026-1035), prepend `new ShellPlugin()` at index 0 **only if** `!allPlugins.some(p => p.name === 'shell')` (covers both `features.shell` and explicit `plugins[]` channels — single instance, no TBW023). Index 0 guarantees shell attaches FIRST (plan invariant 4: claim light-DOM + build shell DOM before first render flush, no flash). One static import + prepend, marked `// SHELL-AUTOREGISTER-V3-370` — the sole v3-deletion point and the ONLY new value import core gains (plan invariant 1).
- DECIDED (Step 1b.1.2a, light-DOM decoupling — what makes "no plugin ⇒ no config change" hold): the THREE shell `registerLightDomHandler('tbw-grid-header'|'tbw-grid-tool-buttons'|'tbw-grid-tool-panel', …)` calls + the `handleShellChange` closure move from core's `#setupLightDomHandlers` into ShellPlugin `attach()`; `unregisterLightDomHandler` for all three in `detach()`. Core keeps ONLY `tbw-grid-column`/`tbw-grid-detail` + the generic `observeLightDOM(this)`. The `MutationObserver` infra stays in `ConfigManager` ([config-manager.ts](libs/grid/src/lib/core/internal/config-manager.ts#L896)) — it is tag-agnostic and already ignores any tag with no registered handler. INVARIANT: with the shell plugin absent, no shell handler is registered ⇒ `<tbw-grid-header>` produces zero callbacks ⇒ zero config/state change.
- DECIDED (Jun 2026, user): unclaimed light-DOM config elements stay VISIBLE (no core fallback hide). The `headerEl.style.display='none'` hide is a side effect of the plugin's `parseLightDomShell`, so it only runs when the plugin is loaded. Plugin absent ⇒ `<tbw-grid-header>`/`<tbw-grid-header-content>` + their text render as raw text above the grid. INTENDED — a visible "this declarative config is doing nothing" signal surfacing a missing-import bug. MUST NOT add a core CSS rule hiding `tbw-grid-*` config tags (would silently reintroduce a core→shell coupling).
- DECIDED (Task 1a.2, type ownership LANDED): shell config types are canonical in [plugins/shell/types.ts](libs/grid/src/lib/plugins/shell/types.ts) — `ShellConfig`, `ShellHeaderConfig`, `ToolPanelConfig`, `ToolbarContentDefinition`, `ToolPanelDefinition`, `HeaderContentDefinition`. Core `GridConfig.shell` removed from the core interface and re-added via `declare module '../../core/types' { interface GridConfig<TRow = any> { /** @deprecated */ shell?: ShellConfig } }` in the plugin (same block as the `PluginNameMap` augmentation). `core/types.ts` imports each canonical type as `*Impl` and re-exports `/** @deprecated */ export type X = XImpl;` (per-symbol tag) so `public.ts` + deep importers keep compiling; aliases deleted at v3. `ShellState`/`ToolPanelRendererFactory` stay in (now-plugin) `shell.ts`; core imports `ShellState` from `'../plugins/shell/shell'`. INVARIANT: augmentation lives in the program ⇒ `config.shell` stays typed during the lib's own typecheck even though core has no intrinsic field. RULED OUT: `export type {X} from` re-export — gives no local binding for `InternalGrid.registerToolPanel(panel: ToolPanelDefinition)` (core/types L177); use `import *Impl` + `export type X = *Impl` instead. Deprecation diagnostic `TBW076` (free) once-per-method-per-grid (dev only). `// TEMP-BUDGET-370` bump in `vite.config.ts` permitted if a transitional shim grows `index.js`; reverted at v3.
- DECIDED (Step 1a.1.1, CSS ownership split): `.tbw-scroll-area` is CORE, not shell. It is built by `dom-builder.ts` `gridContentTemplate` for EVERY grid (shell or not); its siblings (`.rows-body-wrapper`, `.faux-vscroll`, `.tbw-sr-only`) already live in `core/styles/base.css`. It was orphaned in `core/styles/shell.css` → moved to `base.css`, removed from both shell.css copies. `.tbw-grid-content` is shell-only (built solely by `shell.ts` body template as the wrapper around the scroll area) → stays in `plugins/shell/shell.css`. INVARIANT: a shell-less grid gets its scroll-viewport flex/overflow layout from core; the shell plugin owns only `.tbw-shell-*`, `.tbw-grid-content`, `.tbw-toolbar-*`, `.tbw-tool-panel*`, `.tbw-accordion*`.
