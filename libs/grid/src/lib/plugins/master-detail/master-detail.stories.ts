...
let currentMasterDetailArgs: MasterDetailArgs = { animation: 'slide', detailHeight: 'auto', expandOnRowClick: false };

function getMasterDetailSourceCode(args: MasterDetailArgs): string {
  const animationStr = args.animation === false ? 'false' : `'${args.animation}'`;
  const heightStr = args.detailHeight === 'auto' ? "'auto'" : String(args.detailHeight);
...
grid.gridConfig = {
  ...
  plugins: [
    new MasterDetailPlugin({
      ...
      detailHeight: ${heightStr},
      expandOnRowClick: ${args.expandOnRowClick},
      detailRenderer: (row) => {
        const div = document.createElement('div');
        div.style.padding = '16px';
        div.innerHTML = \`<h4>Order Items</h4>\n          <ul>\${row.items.map(i => \`<li>\${i.name} x\${i.qty} — $\${i.price}\`).join('')}</ul>\`;
        return div;
      },
    }),
  ],
...
export const Default: Story = {
  parameters: {
    docs: {
      source: {
        language: 'typescript',
        transform: () => getMasterDetailSourceCode(currentMasterDetailArgs),
      },
    },
  },
  render: (args: MasterDetailArgs) => {
    currentMasterDetailArgs = args;
