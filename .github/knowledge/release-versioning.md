...
- INVARIANT (MIT notice must ship): every publishable package carries its OWN `libs/<pkg>/LICENSE` (copy of the root one) and copies it into `dist/libs/<pkg>/` at build time — `copyLicense()` Vite plugin for grid/react/vue, `"assets": ["./LICENSE"]` in `ng-package.json` for angular. WHY: the root `LICENSE` is outside every package root, so nothing shipped it and the MIT "included in all copies" clause was unmet. `libs/grid/package.json` `files` is an allowlist (`**/*.js`, `**/*.d.ts`, …) → `LICENSE` is listed explicitly there. README badges link `./LICENSE` (relative `../../LICENSE` 404s on npmjs.com). ng-packagr `assets` globs are project-root-relative and cannot escape it — hence the per-lib copy rather than referencing the root file.
