...
Nothing, for a plain trend line. Charting is a domain of its own — scales, axes, tooltips, animation, accessibility — so any built-in version would be either too limited to use or too large to justify. The grid's [`renderer`](/grid/core/#renderers) returns a DOM node, which is all a chart needs, and an inline `<svg>` is a few lines that cost nothing.
