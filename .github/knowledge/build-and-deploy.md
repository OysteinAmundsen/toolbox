...
- INVARIANT (perf self-comparison): CI runs the `e2e` and `docs-e2e` Playwright projects CONCURRENTLY, so a CPU spike can land entirely inside one side of a local-then-cdn sample burst and fake a >1.1 regression that never reproduces locally (`wideColsRender` 1.53, then `columnChurn` 1.59 with the SAME ~220 vs ~145 ms shape, vs ~1.0 across 5-8 local runs). The `REGRESSION_RETRIES` loop does NOT absorb it — a sustained spike covers every attempt, so all 3 "confirm" the fake regression. Any `runComparison` measurement in `e2e/tests/performance-regression.spec.ts` MUST go through `measureInterleaved` (alternates local/cdn sample-by-sample AND flips which page goes first on odd samples), never measure all of one side first. Page-side builders must also clear previous `tbw-grid` elements, since interleaving calls them repeatedly.
