...
- DECIDED (gh #356 phase 6): Angular `FeatureName = Exclude<keyof FeatureConfig, '__brand'>` (parity with React/Vue). Hand-listed union deleted. ng-packagr is taught about every core feature module via a side-effect-free types-only barrel `lib/internal/feature-augmentations.ts` (one `import type {} from '@toolbox-web/grid/features/<name>'` per feature), which `feature-registry.ts` imports the same way. Zero runtime cost (file is elided by tsc), but the modules are in ng-packagr's type-resolution program so the `declare module '../core/types' { interface FeatureConfig {...} }` blocks merge before emit. Verified: emitted `dist/libs/grid-angular/types/toolbox-web-grid-angular.d.ts` contains `type FeatureName = Exclude<keyof FeatureConfig, '__brand'>` (was the hand-listed 25-entry union pre-Phase 6).
