...
| `typeDefaults` | `Record<string, `[`TypeDefault`](/grid/api/core/interfaces/typedefault/)`>` | Default format/renderer per column `type` |
...
| `columnState` | [`GridColumnState`](/grid/api/core/interfaces/gridcolumnstate/) | Saved column state to restore on init |
| `icons` | [`GridIcons`](/grid/api/core/interfaces/gridicons/) | Grid-wide icon overrides |
| `animation` | [`AnimationConfig`](/grid/api/core/interfaces/animationconfig/) | Animation defaults (expand/collapse, reorder, etc.) |
| `loadingRenderer` | [`LoadingRenderer`](/grid/api/core/types/loadingrenderer/) | Custom loading indicator |
| `emptyRenderer` | [`EmptyRenderer`](/grid/api/core/types/emptyrenderer/) ` \| null` | Custom no-rows message (set `null` to suppress) |
| `emptyOverlay` | `'rows' \| 'grid'` | Where to mount the empty overlay (default `'rows'`) |
| `sortHandler` | [`SortHandler`](/grid/api/core/types/sorthandler/) | Low-level sort engine override (prefer `sortComparator` per-column) |
...
| `a11y` | [`A11yConfig`](/grid/api/core/interfaces/a11yconfig/) | Screen reader announcement messages and toggle |
...
By default the grid answers that question itself by reading `row[field]` (and `field` may be a **dotted path** into a nested structure like `deal.capture.instrument`, resolved with no extra config). When you supply a `valueAccessor`, you answer it instead — and **every feature that needs a cell's value reads through your accessor, not just the renderer**: filtering, sorting, aggregation (`sum` / `avg` / … — including the totals shown on group rows), copy-to-clipboard, CSV / Excel export, and the default display all call it. You write the value logic once and the whole grid stays consistent.
