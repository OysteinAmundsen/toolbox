...
function FileExplorer({ files }) {
  return <DataGrid rows={files} gridConfig={gridConfig} />;
}
```

:::note[Alternative: feature prop]
You can instead pass `columns={[ … ]}` and `tree={{ … }}` as individual props on `<DataGrid>`. That form is idiomatic React, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
  <TabItem label="Vue">
```html
...
<template>
  <TbwGrid :rows="files" :grid-config="gridConfig" />
</template>
```

:::note[Alternative: prop + light-dom columns]
You can instead bind `:tree="{ … }"` and declare columns with `<TbwGridColumn>` children. That form is idiomatic Vue, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
...
You can instead import `GridTreeDirective` and bind `[tree]` directly on `<tbw-grid>`. That form gives a compile-time check that the feature is imported, but `gridConfig.features` above is the recommended, framework-consistent approach.
:::
  </TabItem>
</Tabs>
...
Set `loadChildren` to fetch a node's children the first time it is expanded. No other plugin is required — use this when you only need on-demand children, not server-side pagination/sorting/filtering of the root nodes.

Nodes whose children are not yet loaded are marked with a truthy non-array `children` value (e.g. `children: true`). When the data cannot express that, supply a `hasChildren` predicate instead.
...
grid.gridConfig = {
  ...
  features: {
    tree: {
      // Render an expand toggle for folders even before their children load
...
grid.rows = [{ id: 1, name: 'Documents', type: 'folder' }];
```

While the request is in flight the node's toggle is replaced by the grid's small spinner and the row carries `aria-busy="true"`. On success the rows are written to `row[childrenField]`; on failure the node leaves its loading state so collapsing and re-expanding retries the fetch.

`loadChildren` may also return a `Subscribable` (e.g. an Angular `HttpClient` observable) — the subscription is torn down when the grid disconnects, which cancels the underlying request:

```ts
features: {
  tree: {
    loadChildren: ({ row }) => this.http.get(`/api/nodes/${row.id}/children`),
  },
}
```

:::note
`loadChildren` is ignored when `ServerSidePlugin` is active — child data then flows through `dataSource.getChildRows()` instead (see below). A node is fetched at most once; collapsing and re-expanding does not re-request.
:::
    ...
    serverSide: {
      ...
      dataSource: {
        async getRows(params) {
          ...
          return {
            // Nodes may embed `children`, or set it to a truthy non-array
            // marker (e.g. `children: true`) so the expand toggle renders
            // before the children have been fetched.
            rows: data.nodes,
            totalNodeCount: data.total,   // Total top-level nodes on server
          };
        },
        async getChildRows(params) {
          // Called the first time a node is expanded (lazy children only).
          const res = await fetch(`/api/tree/${params.context.parentNode.id}/children`);
...
function ServerTree() {
  return <DataGrid gridConfig={gridConfig} />;
}
```
  </TabItem>
  <TabItem label="Vue">
...
<template>
  <TbwGrid :grid-config="gridConfig" />
</template>
```
  </TabItem>
  <TabItem label="Angular">
...
export class ServerTreeComponent {
  ...
  gridConfig: GridConfig = {
    ...
    features: {
      serverSide: this.serverSideConfig,
      tree: { childrenField: 'children' },
    },
  };
}
...
Fired around lazy child loading — for both the `loadChildren` route and the `ServerSidePlugin` route:

```ts
grid.on('tree-load-start', ({ key, row, depth }) => {
  console.log(`Loading children of ${key}…`);
});

grid.on('tree-load-end', ({ key, childCount }) => {
  console.log(`Loaded ${childCount} children into ${key}`);
});

grid.on('tree-load-error', ({ key, error }) => {
  console.error(`Failed to load children of ${key}`, error);
});
```

See [`TreeLoadStartDetail`](./interfaces/treeloadstartdetail/), [`TreeLoadEndDetail`](./interfaces/treeloadenddetail/), and [`TreeLoadErrorDetail`](./interfaces/treeloaderrordetail/) for the payload types.
