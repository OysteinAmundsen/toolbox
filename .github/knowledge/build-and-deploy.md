...
- INVARIANT: PR job uses `git worktree add ../base $base.sha` to materialise the base commit alongside the PR head. Baseline is measured FIRST (warm-up cost paid before PR head is sampled). Each side is measured **3 times** (`BENCH_ITERATIONS` env var) and merged via `tools/merge-bench-runs.ts` taking **max-of-N on `hz`** (highest = fastest = least polluted by GC/scheduler/fs-cache outliers). Borrowed from Netflix TVUI methodology — outliers on shared CI runners are almost always slower than reality, never faster.
...
- DECIDED (May 2026): `BENCH_ITERATIONS=3` per side (so 6 full bench runs per PR). Sweet spot from Netflix: enough to absorb device-noise outliers without burning CI time. Bump if false-positive rate stays high after a few weeks of data; lower only with strong evidence runner noise has dropped.
- DECIDED (May 2026): on push to main/next, bench runs ONCE (no need for max-of-N when nothing is being compared) and uploads `tmp/bench-current.json` as a 30-day artifact for offline trend tracking. There is no comparison step on push — the same-runner model has no meaningful baseline target.
...
- FLOW (PR): checkout PR head → `git worktree add ../base $base.sha` → in `../base`: `bun install` + N×`vitest bench` per project → in PR head: `bun install` + N×`vitest bench` per project → `merge-bench-runs.ts` produces `tmp/bench-{baseline,current}.json` (max-hz per bench) → `compare-benches.ts --threshold 0.25 --summary "$GITHUB_STEP_SUMMARY"`.
- FLOW (push to main/next, non-release-merge): single `vitest bench` run per project → merge → upload artifact. No comparison.
