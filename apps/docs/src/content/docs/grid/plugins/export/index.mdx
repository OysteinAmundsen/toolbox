...
function EmployeeGrid({ data }) {
  ...
  return (
    <>
      ...
      <DataGrid ref={ref} rows={data} gridConfig={gridConfig} style={{ height: '400px' }} />
    </>
  );
}
```
...
You can instead pass `columns={[ … ]}` and `export={{ … }}` as individual props on `<DataGrid>`. That form is idiomatic React, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
  <TabItem label="Vue">
```html
...
<template>
  <div>
    ...
    <TbwGrid :rows="data" :grid-config="gridConfig" />
  </div>
</template>
```

:::note[Alternative: prop + light-dom columns]
You can instead bind `:export="{ … }"` and declare columns with `<TbwGridColumn>` children. That form is idiomatic Vue, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
...
You can instead import `GridExportDirective` and bind `[exportFeature]` directly on `<tbw-grid>`. That form gives a compile-time check that the feature is imported, but `gridConfig.features` above is the recommended, framework-consistent approach.
:::
  </TabItem>
</Tabs>
...
See [`ExportConfig`](./interfaces/exportconfig/) for the full list of options and defaults.
These set the plugin-wide baseline; every option can be overridden per call via
[`ExportParams`](./interfaces/exportparams/).

### CSV formula escaping

By default, CSV export prefixes any string value beginning with `=`, `+`, `-`, `@`, a tab or a
carriage return with a single quote (`'`). Spreadsheet applications interpret those characters as
the start of a formula, so exporting untrusted row data without this guard lets a value such as
`=cmd|'/c calc'!A1` execute when the file is opened. The quote is not displayed by the spreadsheet.

Set `escapeFormulas: false` in the CSV export options if you are exporting trusted data and need
values to round-trip byte-for-byte:

```ts
grid.exportCsv({ escapeFormulas: false });
```

Excel (SpreadsheetML) export is unaffected — it declares cells as `ss:Type="String"`, so formulas
are never evaluated.
...
See [`ExportParams`](./interfaces/exportparams/) for every per-call option and its default.
Any option omitted from the call falls back to the plugin's
[`ExportConfig`](./interfaces/exportconfig/) value.

Two options change what the export *contains* rather than how it is formatted:

- **`mode`** — `'raw'` (default) writes the underlying typed values; `'formatted'` writes what
  the grid displays, with `column.format` and type defaults applied.
- **`processHeaderRow`** — rewrites plugin-contributed header rows such as column groups.
  Return `null` to blank a cell; a row whose cells are all blank is dropped entirely.

## Column Groups in Exports

**Since 2.10.0.** When the [`GroupingColumnsPlugin`](/grid/plugins/grouping-columns/) is installed alongside the export plugin, column group headers are automatically included above the leaf headers.
...
:::tip[PDF is not a built-in format]
A PDF writer is larger than the whole grid, so it stays out of core — but the accessors above drive one in a dozen lines. See [Recipes: PDF export](/grid/recipes/pdf-export/).
:::
...
Styles resolve in order of increasing specificity: `defaultStyle` → `columnStyles[field]` →
`cellStyle(value, field, row)`. Header cells use `headerStyle` only. See
[`ExcelStyleConfig`](./interfaces/excelstyleconfig/) for the full option list and
[`ExcelCellStyle`](./interfaces/excelcellstyle/) for the shape of an individual style
(`font`, `fill`, `numberFormat`, `alignment`, `borders`).
