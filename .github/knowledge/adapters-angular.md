...
- INVARIANT: features added AFTER the sweep with NO deprecated `Grid` input (e.g. `stickyRows`) MUST still appear as `addPlugin('<name>', undefined)` in `Grid.createFeaturePlugins`, else the directive sets a claim nobody reads. Also append a matching `import type {} from '@toolbox-web/grid/features/<name>';` to `lib/internal/feature-augmentations.ts` so ng-packagr sees the new `FeatureConfig` augmentation in `keyof FeatureConfig`.
- DECIDED (gh #356 phase 6): Angular `FeatureName = Exclude<keyof FeatureConfig, '__brand'>` (parity with React/Vue). Hand-listed union deleted. `lib/internal/feature-augmentations.ts` contains one `import type {} from '@toolbox-web/grid/features/<name>'` per feature; `feature-registry.ts` pulls it in with a **bare side-effect import** (`import './internal/feature-augmentations';` — NOT `import type {} from ...`, which tsc elides and collapses `FeatureName` to `never` → TS2345). The augmentation modules are on ng-packagr's type-resolution program so `declare module '../core/types' { interface FeatureConfig {...} }` blocks merge before emit. Verified: emitted `dist/libs/grid-angular/types/toolbox-web-grid-angular.d.ts` contains `type FeatureName = Exclude<keyof FeatureConfig, '__brand'>` (was the hand-listed 25-entry union pre-Phase 6).
