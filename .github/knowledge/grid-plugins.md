...
- DECIDED (type ownership + non-breaking): shell config types live in `plugins/shell/types.ts` (canonical); core `GridConfig.shell` becomes a `@deprecated` augmentation alias; `core/types.ts`/`public.ts` keep `@deprecated` `export type` re-aliases (zero value coupling, deleted at v3). Deprecation diagnostic code `TBW076` (free) emitted once-per-method-per-grid (dev only). Temporary `// TEMP-BUDGET-370` bump in `vite.config.ts` permitted if a transitional shim grows `index.js` in v2.x; reverted at v3.
