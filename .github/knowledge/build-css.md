...
- INVARIANT (#449, PR 469): "pointer entry" must be enforced, not assumed. Delegated `mouseover` re-fires for EVERY child boundary crossed inside the same cell, and the `cell.title` write dirties style, so a naive handler thrashes layout on each hop. The handler gates on `cell.contains(e.relatedTarget)` before measuring. Any future layout-forcing read behind a delegated `mouseover`/`mouseout` needs the same gate — `relatedTarget` is what turns delegation into `mouseenter` semantics. 2 tests in `event-delegation.spec.ts` → `describe('measures once per cell entry')` count `scrollWidth` reads.
- Same defect existed in `tooltip/tooltip-plugin.ts#onMouseOver` and was worse: per redundant hop it did `isOverflowing` (read) → `#clearAnchor` + re-set `anchor-name`/`aria-describedby` (writes) → `showPopover()` → `#prefersAbove` (`getBoundingClientRect` + `offsetHeight`, a SECOND forced read after the writes invalidated layout), plus a visible re-anchor flicker. `#onMouseOut` in the same class already had the guard — the asymmetry was the tell. Both now share module-level `isSameCellCrossing(cell, related)`. The data-cell branch looked protected by `!dataCell.style.getPropertyValue('anchor-name')`, but that guard means something else (skip overlay editors) and never fires for the majority case where the cell is NOT truncated; the header branch had nothing. 3 tests in `tooltip.spec.ts` → `describe('measures once per pointer entry (#449)')`.
...
- DECIDED (#449): the page is `sidebar: hidden` + `llmsFull: false`, discovered via three in-page links from `accessibility.mdx`. It STAYS in the `llms.txt` index (answers "do you have a VPAT / are you EAA compliant?") but is OUT of the `llms-full.txt` corpus — the 89-row criterion table is noise for code-writing agents.
...
- DECIDED (Jun 2026): supported baseline is **Chrome/Edge 123, Firefox 121, Safari 17.5**. Driven by `light-dark()` (Chrome 123 / FF 120 / Safari 17.5 — 18 uses across `variables.css`, `loading.css`, `empty.css`) and `:has()` (FF 121). Also required: CSS Nesting, `@layer`, `adoptedStyleSheets` (Safari 16.4), `ResizeObserver`/`IntersectionObserver`. Documented in `apps/docs/src/content/docs/grid/platform.mdx#browser-support` + `libs/grid/README.md`.
