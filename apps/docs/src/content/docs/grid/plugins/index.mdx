...
The `@toolbox-web/grid` plugin architecture lets you add capabilities without growing the core bundle. Plugins are tree-shakeable — only import what you need.

:::tip[Features or Plugins? They do the same thing.]
**Features** and **plugins** are two APIs for enabling the same capabilities. The end result is identical — features are a declarative wrapper around plugins.
...
The Angular tabs in plugin docs often show the shorthand `[selection]`, `[filtering]`, `[editing]`, etc. on `<tbw-grid>`. Import the matching per-feature directive (e.g. [`GridSelectionDirective`](/grid/angular/api/directives/gridselectiondirective/) from `@toolbox-web/grid-angular/features/selection`) alongside `Grid` in the component's `imports`. This keeps each feature tree-shakeable. Apps that configure plugins through `[gridConfig]="{ features: { … } }"` don't need the per-feature directives — see the [Angular getting-started guide](/grid/angular/getting-started/#two-ways-to-configure-features) for the full breakdown.
:::
...
| [Row Drag & Drop](/grid/plugins/row-drag-drop/) | Drag-and-drop row reordering and cross-grid row transfer, with keyboard support |
...
| [Sticky Rows](/grid/plugins/sticky-rows/) | Keep matching rows pinned in view while the rest of the grid scrolls |
...
- [Recipes](/grid/recipes/) — Task-oriented how-to guides combining plugins
