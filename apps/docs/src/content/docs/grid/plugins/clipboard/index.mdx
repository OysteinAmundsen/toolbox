...
When `includeHeaders: true`, copied text carries a header row so it round-trips cleanly to spreadsheets and text editors. Pasting **back into cells** drops that header row automatically — only values land in cells, never the header label. The header is stripped only when the pasted first row matches the target columns' labels, so external pastes without a header row are left intact.
