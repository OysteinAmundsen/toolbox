...
import { ServerSidePlugin } from '@toolbox-web/grid/plugins/server-side';
import type { GridConfig, GridElement } from '@toolbox-web/grid';
...
export class ServerSideGridComponent {
  ...
  private plugin = new ServerSidePlugin({ pageSize: 50 });
  ...
  ngAfterViewInit() {
    this.gridRef()?.nativeElement.ready().then(() => {
      this.plugin.setDataSource(this.dataSource);
    });
  }
}
```
