...
- INVARIANT: `grid.query<T>(type, ctx)` ALWAYS returns `T[]` — and `[]` is **truthy**. A yes/no query MUST go through `BaseGridPlugin.queryBoolean(type, ctx)` (reduces with `.some(Boolean)`), never `if (this.grid.query(...))`. DECIDED (#430): this bit twice — `TreePlugin.requestLazyChildren` always took the ServerSide branch so `loadChildren` was never invoked, and `MasterDetailPlugin` always fired `datasource:fetch-children` leaving a permanent detail spinner. Audit any new `query()` call used as a condition. File: [base-plugin.ts](libs/grid/src/lib/core/plugin/base-plugin.ts).
...
- DECIDED (Jun 2026 → Jul 2026, clipboard↔editing decouple, per-cell): clipboard's `defaultPasteHandler` (plugins/clipboard/types.ts) MUST NOT read editing-owned `column.editable`/`rowEditable` directly. Asks `grid.query<CellEditablePredicate>('getCellEditableResolver')` and calls the returned `(field,row)=>boolean` per cell. EditingPlugin declares `{type:'getCellEditableResolver'}` in manifest.queries + answers in `handleQuery` with a predicate wrapping `#isCellEditable` (rowEditable gate + `editable` true/false/function — row-conditional NOW honored). Contract type `CellEditablePredicate` lives in `core/plugin/types.ts` (owned by neither plugin), re-exported from `core/plugin/index.ts` barrel but NOT from package `public.ts` → internal plugin-comm contract, not consumer API. No editing plugin loaded → query unanswered → predicate defaults to `() => false` → paste no-op. HISTORY: replaced the earlier column-level `getEditableFields`→`string[]` query (`GetEditableFieldsContext`), which couldn't express row-conditional editability. One dispatch returns a predicate (not per-cell query dispatch) to keep big-paste cheap. Tests: `getCellEditableResolver` case in `editing-integration.spec.ts`; clipboard mock `query` stub returns a resolver.
- DECIDED (Jul 2026, updateRow routes through data plugins): core `RowManager.updateRow/updateRows` ([row-manager.ts](libs/grid/src/lib/core/internal/row-manager.ts) `#applyRowChanges`) dispatch a `commitCellValue` query PER changed field BEFORE mutating, so programmatic mutations get the same validation/dirty/history/abortion as an interactive edit. Contract `CommitCellValueContext{rowIndex,rowId,field,oldValue,newValue,source}` in `core/plugin/types.ts`; response: `false`=vetoed (skip field), `true`=plugin applied+tracked (core doesn't re-apply), `undefined`=core applies in place + `invalidateAccessorCache` (no-editing/legacy path). Core ALWAYS emits `cell-change` per changed field regardless of handler, then schedules `RenderPhase.VIRTUALIZATION`. EditingPlugin declares `{type:'commitCellValue'}` + answers via `#handleCommitCellValue`: resolves column+row (returns `undefined` if unresolvable → core applies), else runs `#commitCellValue` (cancelable `cell-commit` → dirty → history via `cell-edit-committed` → cascade) and returns `row[field]===newValue` (`false` = vetoed). NEW `UpdateSource` `'history'`: `#handleCommitCellValue` applies value + recomputes dirty but does NOT record history/mark changed — breaks the undo→updateRow→commit→record→undo loop (UndoRedo `#applyValue` calls `updateRow(id,changes,'history')`). INVARIANT: `updateRow` MUST NOT gate editability (server-sync writes read-only cols) — editability filtering stays in clipboard (`getCellEditableResolver`); `commitCellValue` applies regardless. Tests: `editing-integration.spec.ts > programmatic mutation routing via updateRow`.
- DECIDED (Jul 2026, cell-commit re-entrancy self-guard): `#commitCellValue` ([EditingPlugin.ts](libs/grid/src/lib/plugins/editing/EditingPlugin.ts)) tracks in-flight commits in `#committingCells: Set<string>` keyed `` `${rowId ?? rowIndex}\0${field}` ``. It adds the key, emits the cancelable `cell-commit` inside try/finally, then deletes it. A nested commit for a cell ALREADY in the set (a `cell-commit` listener cascading via `detail.updateRow`/`grid.updateRow` back into the SAME cell) RETURNS EARLY without re-applying — the OUTER commit still owns the cell and applies the value + emits `cell-edit-committed` exactly ONCE when its own emit returns. WHY early-return not fall-through: re-applying in the nested call would (a) fire a duplicate `cell-edit-committed` → phantom UndoRedo history entry (UndoRedo auto-records every `cell-edit-committed`, no de-dupe) and (b) sneak the value in even when the outer listener called `preventDefault()`, bypassing the cancelable-commit contract. `updateRow` routes through this pipeline and re-emits `cell-commit`; a listener that calls `updateRow` (to cascade or force a re-render) would otherwise recurse forever (`RangeError: Maximum call stack size exceeded`; fresh timestamps/object refs keep the diff non-convergent). Per-(row,field) keying (not a global boolean) means cascades to OTHER cells still emit `cell-commit` normally, and distinct fields are finite so no unbounded chain. Consumers NO LONGER need their own re-entrancy flag. RULED OUT: adding `source` to `CellCommitDetail` for a consumer-side `source !== 'user'` check — still burdens every consumer; the self-guard needs zero consumer code. Tests: `editing-integration.spec.ts > programmatic mutation routing via updateRow > does not recurse when a cell-commit listener calls updateRow for the same cell` (+ `> honors preventDefault when a listener cascades updateRow to the same cell`).
