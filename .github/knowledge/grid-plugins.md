...
DECIDED (May 2026, #264 WAI-ARIA Treegrid roles for Tree + GroupingRows): when either plugin is registered, swap `.rows-body` `role` from `grid` to `treegrid` and emit `aria-level` / `aria-setsize` / `aria-posinset` on every visible row. WHY: `aria-expanded` on hierarchical rows is only valid inside `treegrid` per WAI-ARIA, and screen readers need level/posinset/setsize to announce "level 2, item 3 of 5". The role swap MUST live in the plugin (not core) — core has no knowledge of hierarchy. Both plugins set the role idempotently in `afterRender` and restore `role="grid"` in `detach()`. Tree carries `posInSet`/`setSize` on `FlattenedTreeRow` (computed in `tree-data.ts#flattenTree` + `TreePlugin.#flattenWithSort`); GroupingRows uses a parallel `flatMeta` array (`computeFlatMeta`) keyed by flat index — group rows resolve their meta in `renderRow` (no `data-row` cell), data rows resolve theirs in `afterRender` via `cell[data-row]`. `aria-multiselectable` is valid on `treegrid` so SelectionPlugin needs no change. Tests: `tree-integration.spec.ts` "WAI-ARIA Treegrid roles (#264)" + `grouping-rows-integration.spec.ts`. Bundle: tree.umd.js +0.4 kB raw / +0 gz; grouping-rows.umd.js +4 kB raw / +0.4 kB gz. INVARIANT: `FlattenedTreeRow.posInSet`/`setSize` is 1-based and per parent (NOT global rowIndex) — fixtures in `tree-datasource.spec.ts#makeFlatRow` must keep both fields.
