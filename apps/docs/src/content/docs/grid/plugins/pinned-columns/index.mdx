...
- **Partial Column Groups support**: Pinned columns work with the
  [Column Groups plugin](../grouping-columns/) when pinning affects whole groups or
  columns in implicit (unlabelled) groups. When a pinned column belongs to an implicit
  group, the group header is automatically split so the pinned portion stays sticky while
  the rest scrolls normally. However, pinning only _some_ columns within an explicit
  (named) column group is not supported—either pin all columns in the group or none.
