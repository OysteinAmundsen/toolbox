...
```
  ...
  <Tab label="Angular">
```typescript
import { Component, ElementRef, ViewChild, CUSTOM_ELEMENTS_SCHEMA, Output, EventEmitter } from '@angular/core';
import '@toolbox-web/grid';
import { ContextMenuPlugin } from '@toolbox-web/grid/plugins/context-menu';
    ...
    this.gridRef.nativeElement.gridConfig = {
      ...
      plugins: [
        new ContextMenuPlugin({
          items: [
            { id: 'copy', label: 'Copy Cell', action: (ctx) => navigator.clipboard.writeText(ctx.value) },
            { id: 'edit', label: 'Edit Row', action: (ctx) => this.gridRef.nativeElement.startEditing(ctx.rowIndex, ctx.colIndex) },
            { type: 'separator' },
