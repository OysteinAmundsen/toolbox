...
Define default renderers for all columns of a specific `type`. This is a **core feature** that works without any plugins.
...
**Resolution Priority**: Column-level `renderer` → Grid `typeDefaults` → App-level (framework adapter) → Built-in

> **Note:** `typeDefaults` also supports `editor` and `editorParams`, but those require the [EditingPlugin](?path=/docs/grid-plugins-editing--docs#type-level-defaults).
