...
| `shell` | [`ShellConfig`](/grid/plugins/shell/) | Shell layout (header, tool panels) — now the [Shell plugin](/grid/plugins/shell/). Configure under `features: { shell }`; the top-level `shell` field is **deprecated** (removed at v3). |
...
In **v2.x** the shell still ships **on by default**, but it is **deprecated**. From **v3.0.0** the shell is opt-in — code that uses the header bar, toolbar, or tool panels **MUST** enable the feature:
...
The top-level `config.shell` field and the `grid.register*` / `openToolPanel` element delegates are **deprecated** and removed at v3. See [Migrating to the feature API](/grid/plugins/shell/#migrating-to-the-feature-api).
:::
