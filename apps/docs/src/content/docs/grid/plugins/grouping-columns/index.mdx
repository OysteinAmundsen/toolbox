...
You can instead pass `columns={[ … ]}` and `groupingColumns={{ … }}` as individual props on `<DataGrid>`. That form is idiomatic React, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
  <TabItem label="Vue">
```html
...
You can instead bind `:grouping-columns="{ … }"` and declare columns with `<TbwGridColumn>` children. That form is idiomatic Vue, but `gridConfig` keeps the configuration identical across every framework.
:::
  </TabItem>
...
You can instead import `GridGroupingColumnsDirective` and bind `[groupingColumns]` directly on `<tbw-grid>`. That form gives a compile-time check that the feature is imported, but `gridConfig.features` above is the recommended, framework-consistent approach.
:::
  </TabItem>
</Tabs>
...
HTML strings returned from the renderer are automatically sanitized (scripts and event-handler attributes are stripped) — see [What the grid sanitizes for you](/grid/guides/platform/#what-the-grid-sanitizes-for-you).
...
<DataGrid rows={data} gridConfig={gridConfig} />
```
  </TabItem>
  <TabItem label="Vue">
```html
...
```
  ...
  <TabItem label="Angular">
Pass a component class as the renderer inside `gridConfig.features`. The adapter bridges it automatically.
