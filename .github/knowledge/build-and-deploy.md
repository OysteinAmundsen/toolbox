...
- OWNS: per-PR perf-regression detection by comparing Vitest `bench()` output (`hz`, `mean`, `moe`, `rme`) of PR head against PR base, both measured **on the same CI runner in a single job** (same VM, kernel, thermal state, neighbour). On push to main/next: bench JSON uploaded as artifact only — no comparison performed (no baseline target in this model).
...
- INVARIANT: PR job uses `git worktree add ../base $base.sha` to materialise the base commit alongside the PR head. Baseline is measured FIRST (warm-up cost paid before PR head is sampled). Each side is measured **N times** (`BENCH_ITERATIONS` env var, currently `2`) and merged via `tools/merge-bench-runs.ts` taking **max-of-N on `hz`** (highest = fastest = least polluted by GC/scheduler/fs-cache outliers). Borrowed from Netflix TVUI methodology — outliers on shared CI runners are almost always slower than reality, never faster. The affected list computed in the PR-head checkout is reused verbatim for the baseline loop, so the same project set is sampled on both sides regardless of whether the base commit knew about new bench files.
- INVARIANT: `merge-bench-runs.ts` rewrites `mean = 1000/hz` for the picked entry so `(hz, mean)` stay coherent — `compare-benches.ts` derives CI bounds from `mean ± moe`, so a picked-from-run-A `hz` paired with a kept-from-run-B `mean` would silently disagree with the threshold gate. `moe` is taken as the **min** across runs (tightest within-run CI seen); we do NOT widen `moe` from cross-run variance because Vitest doesn't expose raw per-iteration samples in the JSON report.
...
- DECIDED (May 2026): same-runner base-vs-head replaced the cached-baseline approach. The previous design cached the most recent main run as baseline and compared PR runs against it; baseline and PR ran on different `ubuntu-latest` VMs so cross-runner variance produced spurious ±30–60% flips on every push. RULED OUT: parallel jobs / matrix / background processes for the two measurements — splitting across runners reverts to the noise problem; same-runner background processes contend for the 4 vCPU and produce worse results than serial. RULED OUT: Netflix-style anomaly + changepoint detection over rolling history of N=40 / N=100 runs — needs persistent storage and significant stats code; way over-engineered for our scale. Same-runner base-vs-head + max-of-3 covers ~the same problem with a fraction of the machinery.
- DECIDED (May 2026): `BENCH_ITERATIONS=2` per side (was 3). Minimum that supports max-of-N; chosen over Netflix's 3 because (a) we have far fewer benches than they do, (b) we run on a single VM (not many devices) so cross-iteration variance is already lower, (c) `nx affected` scoping further bounds the work. Bump back to 3 only with evidence false-positive rate is unacceptable. RULED OUT: shrinking per-bench `time`/`warmupTime` (those are per-`bench()` options in tinybench, not a global vitest config knob; would require touching every `*.bench.ts` and trades looser `moe` against tighter loop — likely undoes some of the noise reduction we just bought).
...
- DECIDED: soft-warn mode — script computes regressions but CI step omits `--fail-on-regression`. With same-runner same-VM measurements + max-of-3, the noise floor should drop substantially; revisit hard-fail after a few PRs of data.
...
- FLOW (PR): checkout PR head → `bun install` → `bun nx show projects --affected --with-target=bench --base=$base.sha --head=$head.sha` → if empty, write `Bench: skipped` to step summary and exit; otherwise `git worktree add ../base $base.sha` → in `../base`: `bun install` + for each affected project: N×`vitest bench` → in PR head: for each affected project: N×`vitest bench` → `merge-bench-runs.ts` produces `tmp/bench-{baseline,current}.json` (max-hz per bench) → `compare-benches.ts --threshold 0.25 --summary "$GITHUB_STEP_SUMMARY"`. Project-name → vite-config-path mapping is a small case statement in the workflow (`grid` → `libs/grid/vite.config.ts`, `grid-react`/`grid-vue` → `vite.config.mts`); unknown names emit `::warning::` and skip.
