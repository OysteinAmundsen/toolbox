...
────────────────────────────────────────────────────────────
  ...
  afterRender()      Grid-wide DOM manipulation
       │
       ▼
  afterCellRender()  Per-cell styling (called for each cell)
       │
       ▼
  afterRowRender()   Per-row styling (called for each row)
       │
       ▼
...
export class HighlightPlugin extends BaseGridPlugin<HighlightConfig> {
  ...
  override afterRender(): void {
    ...
    const cells = gridEl.querySelectorAll(`[data-field="${field}"]`);
    cells.forEach((cell) => {
