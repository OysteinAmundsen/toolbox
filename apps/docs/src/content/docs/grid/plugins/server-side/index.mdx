...
- **[Row Grouping](/grid/plugins/grouping-rows/)** — Inserts group header rows that shift indices, breaking block-based fetching. Row Grouping has its own [async data model](/grid/plugins/grouping-rows/#server-side-integration) for server-backed grouped data.
- **[Tree](/grid/plugins/tree/)** — Requires the full hierarchy to flatten and manage expansion.
- **[Pivot](/grid/plugins/pivot/)** — Requires the full dataset to compute aggregations.
:::
