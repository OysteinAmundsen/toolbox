...
| 📦 **Bundle size** | ~{GRID_STATS.coreGzipKb} kB gzipped, zero runtime dependencies           |
...
:::tip[Paid in AG Grid, free here]
Range selection, clipboard, Excel export, row grouping, pivot tables, tree data, master-detail and the server-side row model all require AG Grid Enterprise. They're MIT here.

See the [full feature comparison](/grid/comparison/).
:::

:::note[Built on stable web standards]
Custom Elements, CSS custom properties and standard DOM APIs. No framework runtime underneath, nothing in `dependencies`, so the grid doesn't inherit anyone else's breaking changes or upgrade schedule.

That matters most if you're weighing abandonment risk. Browser vendors treat removing a shipped, interoperable API as a last resort — it has happened, but on a timescale of decades and with a replacement shipped years ahead. So even if development here stopped tomorrow, a grid you ship today keeps working, through framework majors you never adopt.
:::
...
Plugins get dependency validation, type-safe config extension, and auto-cleanup via `AbortSignal`. They use the same public API third parties do, so you can [build and distribute your own](/grid/plugin-development/custom-plugins/).
