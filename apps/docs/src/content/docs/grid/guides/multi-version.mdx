...
`@toolbox-web/grid` (for example, a host app upgrading from `v2.x` while a
widget is still on `v1.x`), the browser's custom-elements registry is shared
and first-wins: a second `customElements.define('tbw-grid', …)` call throws.
Starting with `v3`, the grid handles this for you automatically — the
...
DataGridElement.version;   // e.g. '3.2.0' — set at build time
...
3. **An existing registration at a DIFFERENT version** → register under
   `tbw-grid-v<sanitised-version>` (e.g. `tbw-grid-v3-2-0`). `activeTag`
...
`3.0.0-beta.1+build.42` produce `tbw-grid-v3-0-0-beta-1-build-42`.
...
<tbw-grid-v2-99-0 data-tbw-grid="">…</tbw-grid-v2-99-0>
```
...
Angular consumers that embed `<tbw-grid>` literally in component templates
work in single-version setups. Multi-version Angular usage requires
rendering the active tag dynamically (e.g. via a structural directive that
reads `DataGridElement.activeTag`).
