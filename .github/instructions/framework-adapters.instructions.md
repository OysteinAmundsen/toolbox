...
- `libs/grid-react/src/lib/portal-manager.tsx` - PortalManager component for context-preserving cell rendering via React portals
- `libs/grid-react/src/lib/portal-bridge.ts` - Module-level bridge connecting non-React adapter code to the PortalManager
...
- **React adapter uses portals, not createRoot** — The React adapter renders cell content via `createPortal()` through a centralized `PortalManager` component mounted inside the `DataGrid` tree. This preserves the full React context chain (Router, Theme, i18n, Redux, etc.). The `portal-bridge.ts` module connects non-React code (adapter, feature files, config wrappers) to the PortalManager. When PortalManager is unavailable, the bridge falls back to `createRoot` (which creates isolated trees without context). Never use `createRoot` directly in new React adapter code — always go through `renderToContainer`/`removeFromContainer` from `portal-bridge.ts`.
