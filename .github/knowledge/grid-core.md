...
- DECIDED (May 2026): always-on focus trap in `FocusManager` (constructed once, listeners on host survive disconnect/reconnect). Tracks the last meaningful user-focused element via capture-phase `focusin` on the grid host + the same `focusin` already wired by `registerExternalFocusContainer`. Restores it on `focusout` when `relatedTarget === null` (bounce-to-body, e.g. an overlay child of `<body>` is removed while focused). `#noteFocus` SKIPS: (a) the grid host itself (artificial `tabindex=0` from keyboard handler), (b) bare `.cell` elements (cell focus is virtual, owned by `_focusRow`/`_focusCol`), (c) descendants of registered external containers (overlay closes → restore should land INSIDE the grid, not back into the about-to-be-removed overlay). Editor inputs inside `.cell.editing` ARE tracked. Restore is gated: `queueMicrotask` → only refocuses when `activeElement` is `<body>` or the grid host. Public surface: `grid.lastFocusedElement` (getter, only returns connected element) + `grid.restoreLastFocus()` (returns `boolean`). `EditingConfig.focusTrap` deprecated to no-op alias — kept on `EditingPlugin` as a safety-net `focusout` listener for the stale-ref edge case (re-render destroys the tracked DOM ref before restore fires; plugin re-finds editor by `_focusRow/_focusCol`). WHY two layers: the trap fixes accidental focus theft (toolpanel `<input>` losing focus on next render, ArrowUp on number-spinner stealing to a cell); the editing plugin fallback covers DOM recreation that invalidates the tracked ref. RULED OUT: calling `focusManager.destroy()` in `disconnectedCallback` (constructor only runs once → reconnect leaves trap uninstalled). `keyboard.ts` `handleGridKeyDown` early-return broadened to ignore ANY non-`.rows-body` target inside the host (toolpanel inputs/buttons, shell-header controls), and `ensureCellVisible` no longer calls `grid.focus()` when a meaningful non-cell descendant currently holds focus. Files: [focus-manager.ts](libs/grid/src/lib/core/internal/focus-manager.ts) (`#installFocusTrap`, `#noteFocus`, `#scheduleRestore`, `restoreLastFocus`), [keyboard.ts](libs/grid/src/lib/core/internal/keyboard.ts), [grid.ts](libs/grid/src/lib/core/grid.ts) (`lastFocusedElement` getter, `restoreLastFocus`), [editing/types.ts](libs/grid/src/lib/plugins/editing/types.ts) (`@deprecated`). Tests: `__tests__/integration/focus-management.spec.ts > always-on focus trap` (6 cases) + `> focusCell preserves non-cell descendant focus` (3 cases).
